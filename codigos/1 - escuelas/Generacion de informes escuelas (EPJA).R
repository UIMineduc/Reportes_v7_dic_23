...
table(bd$desert_glob_total_aju_mayo_2023, bd$desertor_intra, useNA = "ifany")
...
rbds1 <- 1
{
  ...
  # ee = rbds0[10]
  for(ee in pendientes){ #rbds1[1]){     #(ee in quinto1$`0`){   #for(ee in unique(bd_asis$rbd)){ #9810 EEFRANI #3055 #3573 no tiene desvinc  #c(9647, 9648, 9653, 9654, 9655)
    try({   #En caso de que código tenga error el código sigue ejectuándose
      ...
      #asis_crit <- asis_crit %>%
      # select("run_alu2", "nom_alu", "app_alu", "apm_alu", "cod_curso", "rbd", "porcentage_asistencia_marzo",
      #        "porcentage_asistencia_abril","porcentage_asistencia_mayo","porcentage_asistencia_junio",
      #        "porcentage_asistencia_julio","porcentage_asistencia_agosto","porcentage_asistencia_septiembre",
      #        "porcentage_asistencia_octubre","porcentage_asistencia_noviembre","porcentage_asistencia_acumulada",
      #        "inasistencia_grave_acumulada", "asistencia_categorias_acumulada", "tipo_asis_2")
      ...
      asis_crit <- asis_crit %>% select("run_alu2", "nom_alu", "app_alu", "apm_alu",
                                        ...
                                        "porcentage_asistencia_junio","porcentage_asistencia_julio",
                                        "porcentage_asistencia_agosto", "porcentage_asistencia_acumulada",
      ...
      p <- plot_ly(data_plot1, labels = ~sit_fin_r_2022r, values = ~round(prop,3), text = paste0(round(data_plot1$prop*100,1), "%"," <i>(", data_plot1$n,")</i>"), type = 'pie', hole = 0, textposition = 'outside', 
        ...
        layout(legend = list(font = list(family = "verdana", size = 18, color = azul),
                             orientation = "h", x=0.1 , y=1.6),
      ...
      #Sys.setlocale("LC_ALL","Spanish")
      Sys.setlocale("LC_ALL", "ES_ES.UTF-8")
      rmarkdown::render("codigos/1 - escuelas/RMKD Informe Escuela (EPJA).Rmd", 
                        ...
                        output_file = paste0("Outputs/Escuelas 0 (EPJA)/", ee, "-EPJA.pdf"))
...
archivos <- list.files("Outputs/Escuelas 0 (EPJA)")
