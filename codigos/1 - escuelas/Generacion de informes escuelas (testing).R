...
#Editor: "Alonso Arraño - CEM. Noviembre 2023"
...
colnames(bd_desvinc_intran_r2)
 ...
 ## Generación base Maestra (desvinculados inter e intra anuales) ----
...
## Base especial para los particulares pagados
...
# 2. Gestión de BBDD  ----------------------------------------------------------------
#**************************************************************************************************************************/
#"En esta sección se hacen correciones varias a las bbdd tales como:
#Region, Rut, Género, Situación final"

## Ajustes a la base bd y asistencia ----
...
## Ajustes a la base doble_desvinc ----
...
#"Transformación variables de certificacion/validacion estudios en Categórica"
...
## Ajustes 2.0 a la base bd y asistencia ----
...
## Corrección de errores en nombres ----
...
## Idea para generar los vectores
{
...
nn = length(rbds1)
nn
nn=1 #borrar esta linea, SOLO ES UN EJEMPLO!

 #### Parámetros del loop ----
 ##### Función para que el loop sea más automático
"Debes definir el mes:
Abril, Junio, Septiembre o Noviembre"


seleccionar_variables_por_mes <- function(mes) {
  variables_comunes <- c(
    "run_alu2", "nom_alu", "app_alu", "apm_alu", "cod_curso", "rbd",
    "porcentage_asistencia_marzo", "porcentage_asistencia_acumulada",
    "inasistencia_grave_acumulada", "asistencia_categorias_acumulada",
  ...
  if (mes == "Abril") {
    ...
    return(c(variables_comunes, "porcentage_asistencia_abril", "porcentage_asistencia_mayo"))
...
"ingrese nombre mes"
mes <- "Septiembre"
vars_asis_crit <- seleccionar_variables_por_mes(mes)
vars_asis_crit

"ingrese el mes anterior"
mes_data <- "agosto"


 ##### Vars interes desvinculación
{

vars_desvinc=c("run_alu2", "nom_alu2022", "app_alu2022", "apm_alu2022", "gen_alu_2022r", "edad_alu_2022r", "nom_com_alu2022", "nom_grado_2022r", "prom_gral2022", "asistencia2022", "sit_fin_r_2022r", 
...
vars_desvinc2=c("run_alu2_ret", "nom_alu_ret", "app_alu_ret", "apm_alu_ret", "gen_alu_ret", "edad_alu_ret", "nom_com_alu_ret", "nom_grado", "fec_ret_rbd_ret", "valid_estud")

vars_desvinc3=c("run_alu2", "nom_alu", "app_alu", "apm_alu", "gen_alu", "edad_alu", "nom_com_alu", "nom_grado", "sit_fin_r", "rbd", "nom_rbd", "cod_depe2", "cod_reg_rbd", "nom_com_rbd", "cert_val_21", "cert_val_22", "insc_val_23") # cert vald 21? muchos vacios
}

 ##### Plot 5 
param_plot5 <- data.frame(tipo_asis_2 = c("Inasist. crítica (0%-49% asist.)", "Inasist. grave (50%-84% asist.)", "Inasist. reiterada (85%-89% asist.)", "Asist. esperada (90%-100% asist.)", "Sin información"), color = c(rojo, amarillo, celeste, azul, gris))  #azul celeste amarillo rosado rojo gris
param_plot5$tipo_asis_2 <- factor(param_plot5$tipo_asis_2, levels = c("Inasist. crítica (0%-49% asist.)", "Inasist. grave (50%-84% asist.)","Inasist. reiterada (85%-89% asist.)", "Asist. esperada (90%-100% asist.)", "Sin información"), labels =  c("Inasist. crítica (0%-49% asist.)", "Inasist. grave (50%-84% asist.)", "Inasist. reiterada (85%-89% asist.)", "Asist. esperada (90%-100% asist.)", "Sin información"))
param_plot5$tipo_asis_2
...
data_plot5_t_n <- data.frame(tipo_asis_2= c("Inasist. crítica (0%-49% asist.)", "Inasist. grave (50%-84% asist.)", "Inasist. reiterada (85%-89% asist.)", "Asist. esperada (90%-100% asist.)", "Sin información"), n = c(0,0,0,0,0))
 ...
 ##### Plot 5 caso vacio
plot5_t_empty <- data.frame(tipo_asis_2= c("Inasist. crítica (0%-49% asist.)", "Inasist. grave (50%-84% asist.)", "Inasist. reiterada (85%-89% asist.)", "Asist. esperada (90%-100% asist.)", "Sin información"), prop = c(0,0,0,0,0))
plot5_t_n_empty <- data.frame(tipo_asis_2= c("Inasist. crítica (0%-49% asist.)", "Inasist. grave (50%-84% asist.)", "Inasist. reiterada (85%-89% asist.)", "Asist. esperada (90%-100% asist.)", "Sin información"), n = c(0,0,0,0,0))

 ##### GRAFICO SIT_FIN 
param_data_plot1 <- data.frame(sit_fin_r_2022r = c("Promovido", "Reprobado", "Retirado", "Sin Registro"), color = c(azul, celeste, rojo, amarillo))
param_data_plot1$sit_fin_r_2022r <- factor(param_data_plot1$sit_fin_r_2022r, levels =  c("Promovido", "Reprobado", "Retirado", "Sin Registro"), labels =  c("Promovido", "Reprobado", "Retirado", "Sin Registro"))
...

{
  ...
  # ee = rbds0[10]
  for(ee in rbds1[1]){ #rbds1[1]){     #(ee in quinto1$`0`){   #for(ee in unique(bd_asis$rbd)){ #9810 EEFRANI #3055 #3573 no tiene desvinc  #c(9647, 9648, 9653, 9654, 9655)
    try({   #En caso de que código tenga error el código sigue ejectuándose
      ...
      # Dejamos solo estudiantes desertores en `mes` y por si acaso se filtra fallecidos, aunque ya vienen como deserción == 0
      ...
      print(paste0("El establecimiento ",ee," tiene un total de: ",n_desvinc, " alumnos  desvinculados"))
      print(paste0("El establecimiento ",ee," tiene un total de: ",n_desvinc2, " alumnos retirados"))
      print(paste0("El establecimiento ",ee," tiene un total de: ",n_desvinc3, " alumnos doble_desvinc"))
      
      #Los párametros de las variables fueron definidos antes
      desvinc <- desvinc %>% select(all_of(vars_desvinc))
      desvinc <- desvinc %>% arrange(nom_grado_2022r)
      
      desvinc2 <- desvinc2 %>% select(all_of(vars_desvinc2))
      desvinc2 <- desvinc2 %>% arrange(nom_grado)   #(nom_grado_2022r)
      
      desvinc3 <- desvinc3 %>% select(all_of(vars_desvinc3)) # cert vald 21? muchos vacios
      ...
      # NOTA!!: Se modificó la línea con los párametros fuera del loop, asi se reemplaza siempre sin hacer todo el proceso de llamado
      data_plot5 <- param_plot5
      ...
      if (nrow(asis_crit) > 0) {
        ...
        data_plot5_t <- data.frame(
          tipo_asis_2 = names(data_plot5_t),
          prop = as.numeric(data_plot5_t)
        )
        data_plot5_t_n <- data.frame(
          tipo_asis_2 = names(data_plot5_t),
          n = as.numeric(table(asis_crit$tipo_asis_2))
        )
      } else{
        data_plot5_t <- plot5_t_empty
        data_plot5_t_n <- plot5_t_n_empty
        }
      #data_plot5_t
      ...
      asis_crit <- asis_crit %>% select(all_of(vars_asis_crit))
      ...
      #Modificación
      asis_crit <- asis_crit %>%
        mutate_at(vars(starts_with("porcentage_asistencia_")), ~ ifelse(!is.na(.), paste0(round(. * 100, 0), "%"), "-"))
      ...
      # Más rápido que usar orca () por eso usamos kaleido (save_image)
      ...
      rmarkdown::render("RMKD Informe Escuela_nov23.Rmd", 
                        ...
                        output_file = paste0("prueba_", ee, ".pdf")) # Se modifica el Escuelas 0 para editar la carpeta en al que se guardan
...

{
  ...
  for(ee in rbds1){ #rbds1[1]){     #(ee in quinto1$`0`){   #for(ee in unique(bd_asis$rbd)){ #9810 EEFRANI #3055 #3573 no tiene desvinc  #c(9647, 9648, 9653, 9654, 9655)
    try({   #En caso de que código tenga error el código sigue ejectuándose
      ...
      desvinc <- desvinc %>% filter(categoria_desert == "Desercion incidencia")    #(desert_glob_total_aju_marzo_2023 == 1 & fallece_tot == 0)
      desvinc2 <- bd2[bd2$rbd_ret == ee & bd2$categoria_desert == "Retirado 2023", ]
      ...
      desvinc <- desvinc[, vars_desvinc, drop = FALSE]
        desvinc <- desvinc %>% arrange(nom_grado_2022r)
      
      
      desvinc2 <- desvinc2[, vars_desvinc2, drop = FALSE]
        desvinc2 <- desvinc2 %>% arrange(nom_grado)   #(nom_grado_2022r)
      ...
      desvinc3 <- desvinc3[, vars_desvinc3, drop = FALSE]
      desvinc3 <- desvinc3 %>% arrange(nom_grado)
      ...
      asis_crit <- asis_crit %>% filter(inasistencia_grave_acumulada == TRUE)
      n_inasis2023 = nrow(asis_crit)
      print(paste0("El establecimiento ",ee," tiene un total de: ",n_inasis2023, "alumnos con inasistencia_grave"))
     ...
     asis_crit <- asis_crit %>% mutate(asistencia_2022 = ifelse(!is.na(asistencia_2022), paste0(round(asistencia_2022*100,0),"%"), "-"))
      ...
      #Sys.setlocale("LC_ALL","Spanish")
      Sys.setlocale("LC_ALL", "ES_ES.UTF-8")
      render("RMKD Informe Escuela (faltantes).Rmd", 
