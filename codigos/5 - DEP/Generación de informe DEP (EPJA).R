...
resumen_asis_ee <- bd_asis %>%
                  filter(cod_depe2 != 3) %>%
                  group_by(rut_sost_rbd2022, nombre_sost_rbd2022) %>%
...
#Modificacion de los porcentajes de asistencia (agregando el simbolo de %)
...
resumen_asis_ee2 <- bd_asis %>%
  filter(cod_depe2 != 3) %>%
  group_by(rut_sost_rbd2022, nombre_sost_rbd2022, asistencia_categorias_acumulada) %>%
  summarise(n_cat = n())
...
resumen_asis_ee2 <- resumen_asis_ee2 %>% mutate(`NA` = 0)
...
xsl_asis_ee_cur2 <- xsl_asis_ee_cur2 %>% mutate(`NA` = 0)
...
xsl_asis_ee2 <- xsl_asis_ee2 %>% mutate(`NA` = 0)
