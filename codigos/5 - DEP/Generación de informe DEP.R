...
bd_rbd_rutsost2 <- bd_asis %>% group_by(rut_sost_rbd2022, nombre_sost_rbd2022, rbd) %>% summarise(n = n()) %>% ungroup() %>% select(rut_sost_rbd2022, nombre_sost_rbd2022, rbd)
...
doble_desvinc <- left_join(doble_desvinc, bd_rbd_rutsost2, by = c("rbd" = "rbd"))
...
bd_asis <- bd_asis %>% filter(cod_depe2 == 5)


data_plot4_n <- bd %>% group_by(rut_sost_rbd2022, nombre_sost_rbd2022) %>% summarise(n_total = n()) %>% ungroup()  #**
data_plot4_n_2 <- bd_asis %>% group_by(rut_sost_rbd2022, nombre_sost_rbd2022) %>% summarise(n_total = n())  %>% ungroup()   #**
data_plot6_n <- bd_asis %>% group_by(rut_sost_rbd2022, nombre_sost_rbd2022) %>% summarise(n_total = n()) %>% ungroup()   #**
...
resumen_asis_ee <- bd_asis %>% filter(cod_depe2 != 3) %>% group_by(rut_sost_rbd2022, nombre_sost_rbd2022) %>% summarise(asis_marzo= mean(porcentage_asistencia_marzo, na.rm = TRUE), 
...
resumen_asis_ee2 <- bd_asis %>% filter(cod_depe2 != 3) %>% group_by(rut_sost_rbd2022, nombre_sost_rbd2022, asistencia_categorias_acumulada) %>% summarise(n_cat = n())
...
resumen_asis_ee = left_join(resumen_asis_ee, resumen_asis_ee2, by = c("rut_sost_rbd2022", "nombre_sost_rbd2022"))
...
resumen_asis_ee <- resumen_asis_ee %>% mutate(porc1 = ifelse(!is.na(`1`), round(`1`/n_total2, 3), 0))
resumen_asis_ee <- resumen_asis_ee %>% mutate(porc2 = ifelse(!is.na(`2`), round(`2`/n_total2, 3), 0))
resumen_asis_ee <- resumen_asis_ee %>% mutate(porc3 = ifelse(!is.na(`3`), round(`3`/n_total2, 3), 0))
resumen_asis_ee <- resumen_asis_ee %>% mutate(porc4 = ifelse(!is.na(`4`), round(`4`/n_total2, 3), 0))
resumen_asis_ee <- resumen_asis_ee %>% mutate(sin_info = ifelse(!is.na(`NA`), round(`NA`/n_total2, 3), NA))
...
n_ret_rbd <- bd2 %>%  group_by(rut_sost_rbd2022_ret) %>% summarise(n_ret = sum(retirados, na.rm = T)) %>% ungroup() %>% select(rut_sost_rbd2022_ret, n_ret)
data_plot4_n_2 <- left_join(data_plot4_n_2, n_ret_rbd, by=c("rut_sost_rbd2022" = "rut_sost_rbd2022_ret"))
...
# bd_asis$nombr
mat_2023_slep <- bd_asis %>% group_by(rut_sost_rbd2022, nombre_sost_rbd2022) %>% summarise(n_total23 = n()) %>% ungroup()
...
resumen_desvinc_ee <- bd %>% group_by(rut_sost_rbd2022) %>% summarise(n_total = n(), n_desv = sum(desvinculados))#summarise(n_total = n(), n = sum(desert_glob_total_aju_marzo_2023))
resumen_desvinc_ee2 <- bd2 %>% group_by(rut_sost_rbd2022_ret) %>% summarise(n_ret = sum(retirados, na.rm = T)) %>% ungroup() %>% select(rut_sost_rbd2022_ret, n_ret)#summarise(n_total = n(), n = sum(desert_glob_total_aju_marzo_2023))
resumen_dobledesvinc_ee <- doble_desvinc %>% group_by(rut_sost_rbd2022) %>% summarise(n_dobledesv = n()) %>% ungroup() %>% select(rut_sost_rbd2022, n_dobledesv)#summarise(n_total = n(), n = sum(desert_glob_total_aju_marzo_2023))

resumen_desvinc_ee <- full_join(mat_2023_slep, resumen_desvinc_ee, by = c("rut_sost_rbd2022" = "rut_sost_rbd2022"))
resumen_desvinc_ee <- full_join(resumen_desvinc_ee, resumen_desvinc_ee2, by = c("rut_sost_rbd2022" = "rut_sost_rbd2022_ret"))
resumen_desvinc_ee <- left_join(resumen_desvinc_ee, resumen_dobledesvinc_ee, by = c("rut_sost_rbd2022" = "rut_sost_rbd2022"))
...
xls_desvinc_ee <- xls_desvinc_ee %>% select(rut_sost_rbd2022, nombre_sost_rbd2022, rbd_2022r, nom_rbd, cod_reg_rbd2022r2, nom_com_rbd, n_mat_teo_total23, desv_total23, porc_total23, n_ret, n_desv, n_dobledesv)



#DESVINCULADOS POR RBD Y CURSO
xls_desvinc_ee_cur <- xls_desvinc_ee_cur %>% select(rut_sost_rbd2022, nombre_sost_rbd2022, rbd_2022r, nom_rbd, cod_reg_rbd2022r2, nom_com_rbd, nom_grado, desv_total23, n_ret, n_desv, n_dobledesv)
...
resumen_desvinc_ee_aux <- resumen_desvinc_ee  %>% select(-c(nombre_sost_rbd2022)) %>% rename(n_total22 = n_total) # 
resumen_desvinc_ee <- resumen_desvinc_ee  %>% rename(n_total22 = n_total) 
resumen_rbd <- left_join(resumen_asis_ee, resumen_desvinc_ee_aux, by = c("rut_sost_rbd2022"="rut_sost_rbd2022"))
resumen_rbd <- resumen_rbd %>% arrange(nombre_sost_rbd2022)
colnames(resumen_rbd)
(resumen_rbd)
resumen_rbd <- resumen_rbd %>% filter(!is.na(rut_sost_rbd2022))
...
colnames(resumen_rbd) <- c("rut_sost_rbd2022", "Nombre SLEP", "Asistendia promedio marzo", "Asistendia promedio abril", "Asistendia promedio mayo",
                           ...
                           "Matrícula 2023 + estud. sin matrícula", "N° Estud. 2022-2023 sin matricula vigente", "% Estud. 2022-2023 sin matricula vigente", "N° Estud. 2023 retirados en el año y sin matrícula vigente", "N° Estud. 2022 no matriculados en 2023", "Detalle N° Estud. 2021 no matriculados ni en 2022 ni 2023")


# colnames(resumen_rbd) <- c("rut_sost_rbd2022", "Nombre SLEP", "RBD", "Nombre establecimiento", "Comuna", "Asistencia promedio marzo", "Asistencia promedio acumulada", 
...

{
  ...
  #######
  #rut_sost = 62000660
  #for(rut_sost in ruts_sost[1]){     #rutsost1 #c(42,53,101,136,134)
  try({
    tic()
    #**************************************************************************************************************************/
    ## 3.1 PRE RENDER  ----------------------------------------------------------------
    ...
    # #**************************************************************************************************************************/
    #
    rmarkdown::render("Codigos/DEP/RMKD Informe DEP.Rmd",
                      ...
                      output_file = paste0("Outputs/DEP/", "DEP", ".pdf"))
    ...
    addStyle(wb, "Resumen asist x RBD y grado", style=pct, cols=c(7:13, 16, 22:26), rows=2:(nrow(xsl_asis_ee_cur_deprov)+1), gridExpand=TRUE)
    
    saveWorkbook(wb, paste0("Outputs/DEP/","DEP",".xlsx"), overwrite = T)
