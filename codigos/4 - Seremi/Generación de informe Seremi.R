...
textos_variables_doble_desvinculados <- openxlsx::read.xlsx("Inputs/Tabla variables.xlsx", sheet = "Doble_desvinculados")
 ...
 bd_desvinc_intran$cod_reg_rbd <- factor(bd_desvinc_intran$cod_reg_rbd, levels = c(1:16), labels = c("Región de Tarapacá",  "Región de Antofagasta", "Región de Atacama", "Región de Coquimbo", "Región de Valparaíso",  "Región de O'Higgins", "Región del Maule", 
                                                                                    "Región del Biobío", "Región de La Araucanía", "Región de Los Lagos", "Región de Aysén", "Región de Magallanes", "Región Metropolitana","Región de Los Ríos","Región de Arica y Parinacota", "Región del Ñuble"))
 

 
 head(bd_desvinc_intran$run_alu2)
 table(bd_desvinc_intran$cod_reg_rbd, bd_desvinc_intran$nom_reg_rbd_a, useNA = "a")
 ...
 #Se agrega nombre grado a los retirados   #tagnuevo (1)  
 bd_desvinc_intran <- left_join(bd_desvinc_intran, nom_grad, by = c("cod_ense" = "cod_ense", "cod_grado" = "cod_grado"))
 
 #Se selecciona rut y columnas de deserción para hacer la comparación          #tagnuevo(1) --> listo
 bd_desvinc_intran_r <- bd_desvinc_intran %>% select("run_alu", "validacion_estudios",  "desertor_intra")  # select(2, 67, 68, 70, 71)   
colnames(bd_desvinc_intran)
 ...
 bd_desvinc_intran_r2 <- bd_desvinc_intran %>% select("run_alu", "nom_alu", "app_alu", "apm_alu", "gen_alu", "edad_alu", "nom_com_alu", "cod_grado", "cod_ense", "nom_grado","fec_ret_rbd", "run_alu2",
                                                      "rbd", "nom_rbd", "cod_depe2", "cod_reg_rbd", "cod_com_rbd", "nom_com_rbd", "cod_deprov_rbd", "nom_deprov_rbd") #tagnuevo se agregan campos de retirados
 
 #Se le pega rut de sostenedor a la bd de retirados, ya que no lo tiene    #tagdiferencia (4)
 bd_rbd_rutsost <- bd_asis %>%
   group_by(rut_sost_rbd2022, nombre_sost_rbd2022, rbd) %>%
   slice(1) %>%
   ungroup() %>%
   select(rut_sost_rbd2022, nombre_sost_rbd2022, email_sost_rbd2022, tel_sost_rbd, rbd)  #tagnuevo se agregan campos de retirados
 ...
 bd <- bd %>% mutate(categoria_desert = ifelse(desert_glob_total_aju_agosto_2023 == 1 & desertor_intra == 1, "Retirado 2023", "otro"))
 bd <- bd %>% mutate(categoria_desert = ifelse(desert_glob_total_aju_agosto_2023 == 0, "No desertor", categoria_desert))
 bd <- bd %>% mutate(categoria_desert = ifelse(desertor_intra == 1 , "Retirado 2023", categoria_desert))
 bd <- bd %>% mutate(categoria_desert = ifelse(desert_glob_total_aju_agosto_2023 == 1 & is.na(desertor_intra), "Desercion incidencia", categoria_desert))
 ...
 bd <- bd %>% mutate(gen_alu_2022r = case_when(gen_alu_2022r == 1 ~ "M",
                                               gen_alu_2022r == 2 ~ "F",
                                               gen_alu_2022r == 0 ~ "",
                                               TRUE ~ ""))
 #table(bd$cod_reg_rbd2022r2, bd$cod_reg_rbd2022r)
 
 bd <- bd %>% mutate(sit_fin_r_2022r = case_when(is.na(sit_fin_r_2022r) | sit_fin_r_2022r == "" ~ "Sin Registro", 
                                                 sit_fin_r_2022r == "P" ~ "Promovido",
                                                 sit_fin_r_2022r == "R" ~ "Reprobado",
                                                 sit_fin_r_2022r == "Y" ~ "Retirado"))
 ...
 doble_desvinc <- doble_desvinc %>% mutate(sit_fin_r = case_when(is.na(sit_fin_r) | sit_fin_r == "" | sit_fin_r == "Z" ~ "Sin Registro",
                                                                 sit_fin_r == "P" ~ "Promovido",
                                                                 sit_fin_r == "R" ~ "Reprobado",
                                                                 sit_fin_r == "Y" ~ "Retirado"))
 ...
 doble_desvinc$cod_reg_rbd <- factor(doble_desvinc$cod_reg_rbd, levels = c(1:16), labels = c("Región de Tarapacá",  "Región de Antofagasta", "Región de Atacama", "Región de Coquimbo", "Región de Valparaíso",  "Región de O'Higgins", "Región del Maule", 
                                                                                 "Región del Biobío", "Región de La Araucanía", "Región de Los Lagos", "Región de Aysén", "Región de Magallanes", "Región Metropolitana","Región de Los Ríos","Región de Arica y Parinacota", "Región del Ñuble"))
 ...
 bd_asis$cod_reg_rbd <- factor(bd_asis$cod_reg_rbd, levels = c(1:16), labels = c("Región de Tarapacá",  "Región de Antofagasta", "Región de Atacama", "Región de Coquimbo", "Región de Valparaíso",  "Región de O'Higgins", "Región del Maule", 
                                                                                                     "Región del Biobío", "Región de La Araucanía", "Región de Los Lagos", "Región de Aysén", "Región de Magallanes", "Región Metropolitana","Región de Los Ríos","Región de Arica y Parinacota", "Región del Ñuble"))
 
 # bd_asis <- bd_asis %>% rename(cod_reg_rbd2022r2 = cod_reg_rbd)
 ...
 bd_asis <- bd_asis %>%
   mutate(tipo_asis_2 = case_when(asistencia_categorias_acumulada == 4 ~ 'Asist. esperada (90%-100% asist.)',
                                  asistencia_categorias_acumulada == 3 ~ 'Inasist. reiterada (85%-89% asist.)',
                                  asistencia_categorias_acumulada == 2 ~ 'Inasist. grave (50%-84% asist.)',
                                  asistencia_categorias_acumulada == 1 ~ 'Inasist. crítica (0%-49% asist.)',
                                  is.na(asistencia_categorias_acumulada) ~ 'Sin información'))
 
 bd_asis <- bd_asis %>% rename(cod_reg_rbd2022r2 = cod_reg_rbd)
 #tagdiferencia (2)  --> listo
 #Se pegan los ruts de sost 2023 a la bd 2022
 bd <- bd %>% select(-c(rut_sost_rbd2022, nombre_sost_rbd2022, email_sost_rbd2022,tel_sost_rbd))
...
# Corrección de errores #tagdiferencia  -> listo
...
#Se guarda la bd de asistencia para tener le matrícula de todos los ee, incluidos los pp
bd_asis_todos <- bd_asis

# #Se filtran ee que sean pp
#bd <- bd %>% filter(cod_depe2_2022r != 3)

data_plot4_n <- bd %>% group_by(cod_com_rbd_2022r, nom_com_rbd_2022r) %>% summarise(n_total = n()) %>% ungroup()  #**
data_plot4_n_2 <- bd_asis %>% group_by(cod_com_rbd, nom_com_rbd) %>% summarise(n_total = n()) %>% ungroup()   #**
bd_asis <- bd_asis %>% filter(cod_depe2 != 3)
data_plot6_n <- bd_asis %>% group_by(cod_com_rbd, nom_com_rbd) %>% summarise(n_total = n()) %>% ungroup()   #**
...
resumen_asis_ee <- bd_asis %>%
                    filter(cod_depe2 != 3) %>%
                    filter(!is.na(inasistencia_grave_acumulada)) %>%
                    group_by(cod_reg_rbd2022r2, nom_com_rbd, cod_com_rbd, cod_depe2) %>% summarise(asis_marzo= mean(porcentage_asistencia_marzo, na.rm = TRUE), 
...
resumen_asis_ee <- resumen_asis_ee %>% arrange(asis_promedio)

#tagnuevo (1)
# resumen_asis_ee_aju <- resumen_asis_ee 
...
resumen_asis_ee2 <- bd_asis %>% filter(cod_depe2 != 3) %>% group_by(cod_reg_rbd2022r2, nom_com_rbd, cod_com_rbd, cod_depe2, asistencia_categorias_acumulada) %>% summarise(n_cat = n())
...
resumen_asis_ee = left_join(resumen_asis_ee, resumen_asis_ee2, by = c("cod_reg_rbd2022r2", "nom_com_rbd", "cod_com_rbd", "cod_depe2"))

#tagnuevo (1)
# resumen_asis_ee_aju = left_join(resumen_asis_ee_aju, resumen_asis_ee2, by = c("cod_reg_rbd2022r2", "nom_com_rbd", "cod_com_rbd", "cod_depe2"))
# table(resumen_asis_ee_aju$cod_reg_rbd2022r2, resumen_asis_ee_aju$nom_com_rbd, useNA = "a")

resumen_asis_ee
resumen_asis_ee <- resumen_asis_ee %>% rowwise() %>% mutate(n_total2 = sum(`1`, `2`, `3`, `4`, `NA`, na.rm = T))
#tagnuevo (1)
# resumen_asis_ee_aju <- resumen_asis_ee_aju %>% rowwise() %>% mutate(n_total2 = sum(`1`, `2`, `3`, `4`, `5`, na.rm = T))
...
resumen_asis_ee <- resumen_asis_ee %>% mutate(sin_info = ifelse(!is.na(`NA`), paste0(100*round(`NA`/n_total2, 3),"%"), NA))
resumen_asis_ee <- resumen_asis_ee %>% arrange(nom_com_rbd, cod_depe2)

#tagnuevo (4)
# resumen_asis_ee_aju <- resumen_asis_ee_aju %>% mutate(porc1 = ifelse(!is.na(`1`), round(`1`/n_total2, 3),0))
# resumen_asis_ee_aju <- resumen_asis_ee_aju %>% mutate(porc2 = ifelse(!is.na(`2`), round(`2`/n_total2, 3),0))
# resumen_asis_ee_aju <- resumen_asis_ee_aju %>% mutate(porc3 = ifelse(!is.na(`3`), round(`3`/n_total2, 3),0))
# resumen_asis_ee_aju <- resumen_asis_ee_aju %>% mutate(porc4 = ifelse(!is.na(`4`), round(`4`/n_total2, 3),0))
# resumen_asis_ee_aju <- resumen_asis_ee_aju %>% mutate(porc5 = ifelse(!is.na(`5`), round(`5`/n_total2, 3),0))
...
colnames(bd2)
#Se calculan los retirados por rbd #tagnuevo (5)
n_ret_rbd <- bd2 %>%  group_by(cod_com_rbd_ret) %>% summarise(n_ret = sum(retirados, na.rm = T)) %>% ungroup() %>% select(cod_com_rbd_ret, n_ret)
data_plot4_n_2 <- left_join(data_plot4_n_2, n_ret_rbd, by=c("cod_com_rbd" = "cod_com_rbd_ret"))
...
mat_2023_com_cur <- bd_asis_todos %>% group_by(cod_reg_rbd2022r2, nom_com_rbd, cod_com_rbd, cod_depe2) %>% summarise(n_total23 = n()) %>% ungroup()
mat_2023_com_cur <- mat_2023_com_cur %>% rename(cod_reg_rbd2022r2 = cod_reg_rbd2022r2, nom_com_rbd_2022r = nom_com_rbd, cod_com_rbd_2022r = cod_com_rbd, cod_depe2_2022r =  cod_depe2)



colnames(bd_asis_todos)
...
resumen_desvinc_ee <- bd  %>%  group_by(cod_com_rbd_2022r, cod_depe2_2022r) %>% summarise(n_total = n(), n_desv = sum(desvinculados, na.rm = T))#summarise(n_total = n(), n = sum(desert_glob_total_aju_marzo_2023))
resumen_desvinc_ee2 <- bd2 %>% group_by(cod_com_rbd_ret, cod_depe2_ret) %>% summarise(n_ret = sum(retirados, na.rm = T)) %>% ungroup() #%>% select(cod_reg_rbd_ret, n_ret) #summarise(n_total = n(), n = sum(desert_glob_total_aju_marzo_2023))
resumen_dobledesvinc_ee <- doble_desvinc %>% group_by(cod_com_rbd, cod_depe2) %>% summarise(n_dobledesv = n()) #%>% ungroup() %>% select(rbd, n_dobledesv)#summarise(n_total = n(), n = sum(desert_glob_total_aju_marzo_2023))

resumen_desvinc_ee <- full_join(mat_2023_com_cur, resumen_desvinc_ee, by = c("cod_com_rbd_2022r" = "cod_com_rbd_2022r", "cod_depe2_2022r" =  "cod_depe2_2022r"))
resumen_desvinc_ee <- full_join(resumen_desvinc_ee, resumen_desvinc_ee2, by = c("cod_com_rbd_2022r" = "cod_com_rbd_ret", "cod_depe2_2022r" =  "cod_depe2_ret"))
resumen_desvinc_ee <- left_join(resumen_desvinc_ee, resumen_dobledesvinc_ee, by = c("cod_com_rbd_2022r" = "cod_com_rbd", "cod_depe2_2022r" =  "cod_depe2"))
...
resumen_desvinc_ee <- resumen_desvinc_ee %>% arrange(nom_com_rbd_2022r, cod_depe2_2022r)
...
#---------
 ...
 table(bd$desvinculados, useNA = "a")
 
 
 
 mat_2023_ee <- bd_asis_todos %>% group_by(rbd) %>% summarise(n_total23 = n()) %>% ungroup()
 mat_2023_ee_cur <- bd_asis_todos %>% group_by(rbd, cod_curso) %>% summarise(n_total23 = n()) %>% ungroup()
 mat_2023_ee <- mat_2023_ee %>% rename(rbd_2022r = rbd)
 mat_2023_ee_cur <- mat_2023_ee_cur %>% rename(nom_grado = cod_curso, rbd_2022r = rbd)

 datos_rbd2023 <- bd_asis_todos %>% group_by(rbd) %>% slice(1) %>% select(rbd, cod_depe2, nom_rbd, cod_deprov_rbd, nom_deprov_rbd, cod_com_rbd, nom_com_rbd, cod_reg_rbd2022r2, nombre_sost_rbd2022, rut_sost_rbd2022, email_sost_rbd2022, tel_sost_rbd)
 datos_rbd2023 <- datos_rbd2023  %>% rename(cod_reg_rbd2022r2 = cod_reg_rbd2022r2)
 
 datos_rbd2022 <- bd %>% group_by(rbd_2022r) %>% slice(1) %>% select(rbd_2022r, nom_deprov_rbd2022, cod_deprov_rbd2022, cod_depe2_2022r, nom_rbd_2022r, cod_com_rbd_2022r, nom_com_rbd_2022r, cod_reg_rbd2022r2, nombre_sost_rbd2022, rut_sost_rbd2022, email_sost_rbd2022, tel_sost_rbd)
 colnames(datos_rbd2022) <- paste0(colnames(datos_rbd2022),"_t_1")
 #datos_rbd2023_ret <- bd2 %>% group_by(rbd_ret) %>% slice(1) %>% select(rbd_ret, cod_depe2_ret, nom_rbd_ret, nom_deprov_rbd_ret, nom_com_rbd_ret, nombre_sost_rbd2022_ret, rut_sost_rbd2022_ret, email_sost_rbd2022_ret, tel_sost_rbd_ret)
...
#xls_desvinc_ee <- bd %>%  group_by(rbd_2022r) %>%  summarise(n_total = n(), n_desv = sum(desvinculados, na.rm = T)) 
#xls_desvinc_ee2 <- bd2  %>%  group_by(cod_reg_rbd_ret, rbd_ret) %>% summarise(n_ret = sum(retirados, na.rm = T)) %>% ungroup() %>% select(cod_reg_rbd_ret, rbd_ret, n_ret)
...
# xls_desvinc_ee <- xls_desvinc_ee %>% arrange(n_desv)
...
#print(xls_desvinc_ee)

#ACA(3)
#Se pegan los datos de matrícula 2023 a la bd de desvinculados y retirados


# sum(mat_2023_ee$n_total23, na.rm = T)
# sum(mat_2023_ee_cur$n_total23, na.rm = T)
...
xsl_asis_ee_cur <- xsl_asis_ee_cur %>% mutate(sin_info = ifelse(!is.na(`NA`), round(`NA`/n_total2, 3), NA))      #ifelse(!is.na(`5`), paste0(100*round(`5`/n_total2, 3),"%"), "0%"))
...
xsl_asis_ee <- xsl_asis_ee %>% mutate(sin_info = ifelse(!is.na(`NA`), round(`NA`/n_total2, 3), NA))      #ifelse(!is.na(`5`), paste0(100*round(`5`/n_total2, 3),"%"), "0%"))
...
xsl_asis_ee <- xsl_asis_ee %>% arrange(nom_com_rbd, rbd)
xsl_asis_ee_cur <- xsl_asis_ee_cur %>% arrange(nom_com_rbd, rbd)
xls_desvinc_ee <- xls_desvinc_ee %>% arrange(nom_com_rbd , rbd_2022r)
xls_desvinc_ee_cur <- xls_desvinc_ee_cur %>% arrange(nom_com_rbd , rbd_2022r)
...
#ASISTENCIA POR RBD
xsl_asis_ee <- xsl_asis_ee %>% select(cod_reg_rbd2022r2, cod_deprov_rbd, nom_deprov_rbd, nom_com_rbd, nombre_sost_rbd2022, rut_sost_rbd2022,
                                                    email_sost_rbd2022, tel_sost_rbd, cod_depe2, rbd, nom_rbd, asis_marzo, 
                                                    asis_abril, asis_mayo,
                                                    asis_junio, asis_julio, asis_agosto,
                                                    #asis_septiembre, asis_octubre, asis_noviembre,
                                                    asis_promedio, n_total, n, porc_asis_crit, "1", "2", "3", "4", "NA", porc1, porc2, porc3,
                                                    porc4, sin_info) # n_total2


#ASISTENCIA POR RBD Y CURSO
xsl_asis_ee_cur <- xsl_asis_ee_cur %>% select(cod_reg_rbd2022r2, cod_deprov_rbd, nom_deprov_rbd, nom_com_rbd, nombre_sost_rbd2022, rut_sost_rbd2022,
                                                            email_sost_rbd2022, tel_sost_rbd, cod_depe2, rbd, nom_rbd, cod_curso, asis_marzo, 
                                                            asis_abril, asis_mayo,
                                                            asis_junio, asis_julio, asis_agosto,
                                                            #asis_septiembre, asis_octubre, asis_noviembre,
                                                            asis_promedio, n_total, n, porc_asis_crit, "1", "2", "3", "4", "NA", porc1, porc2, porc3,
...
xls_desvinc_ee <- xls_desvinc_ee %>% select(cod_reg_rbd2022r2, cod_deprov_rbd, nom_deprov_rbd, nom_com_rbd, nombre_sost_rbd2022, rut_sost_rbd2022, email_sost_rbd2022, 
                                                          tel_sost_rbd, cod_depe2_2022r, rbd_2022r, nom_rbd, n_mat_teo_total23, desv_total23, porc_total23, n_ret, n_desv, n_dobledesv)

#xls_desvinc_ee %>% filter(rbd_2022r == 12634)

#DESVINCULADOS POR RBD Y CURSO
xls_desvinc_ee_cur<- xls_desvinc_ee_cur %>% select(cod_reg_rbd2022r2, cod_deprov_rbd, nom_deprov_rbd, nom_com_rbd, nombre_sost_rbd2022, rut_sost_rbd2022, email_sost_rbd2022, 
                                                                  tel_sost_rbd, cod_depe2_2022r, rbd_2022r, nom_rbd, nom_grado, desv_total23, n_ret, n_desv, n_dobledesv)
...
tbl_asis_ee <- xsl_asis_ee %>% select(cod_reg_rbd2022r2, nom_com_rbd, cod_depe2, rbd, nom_rbd, nombre_sost_rbd2022, asis_marzo, 
                                      asis_abril, asis_mayo,
                                      asis_junio, asis_julio, asis_agosto,
                                      #asis_septiembre, asis_octubre, asis_noviembre,
  ...
  resumen_rbd <- resumen_rbd %>% arrange(nom_com_rbd, cod_depe2, nom_rbd)
  ...
  colnames(resumen_rbd) <- c("cod_reg_rbd2022r2", "Comuna", "Dependencia", "RBD", "Nombre establecimiento", "Nombre Sostenedor", "Asist. promedio marzo", 
                             "Asist. promedio abril", "Asist. promedio mayo",
                             "Asist. promedio junio", "Asist. promedio julio", "Asist. promedio agosto",
                             #"Asist. promedio sept.", "Asist. promedio octubre", "Asist. promedio nov.",
                             "Asist. promedio acumulada", "Matrícula 2023", "N° estud. con asistencia <85%", "% estud. con asistencia <85%", "Matrícula 2023 + estud. sin matrícula", "N° Estud. 2022-2023 sin matricula vigente", "% Estud. 2022-2023 sin matricula vigente", "N° Estud. 2023 retirados en el año y sin matrícula vigente", "N° Estud. 2022 no matriculados en 2023", "Detalle N° Estud. 2021 no matriculados ni en 2022 ni 2023")

  
  colnames(xsl_asis_ee_cur) <- c("cod_reg_rbd2022r2", "cod_deprov_rbd", "Nombre Deprov", "Comuna", "Nombre Sostenedor", "RUT Sostenedor", "email Sostenedor", "Teléfono Sostenedor", "Dependencia", "RBD", "Nombre RBD", "Grado", "Asistencia promedio marzo", 
  ...
  colnames(xsl_asis_ee) <- c("cod_reg_rbd2022r2", "cod_deprov_rbd", "Nombre Deprov", "Comuna", "Nombre Sostenedor", "RUT Sostenedor", "email Sostenedor", "Teléfono Sostenedor", "Dependencia", "RBD", "Nombre RBD", "Asistencia promedio marzo", 
                             "Asistendia promedio abril", "Asistendia promedio mayo",
  ...
  colnames(xls_desvinc_ee) <- c("cod_reg_rbd2022r2", "cod_deprov_rbd2022", "Nombre Deprov", "Comuna", "Nombre Sostenedor", "RUT Sostenedor", "email Sostenedor", "Teléfono Sostenedor", "Dependencia", "RBD", "Nombre RBD", "Matrícula 2023 + estudiantes sin matrícula", "N° Estudiantes 2022-2023 sin matricula vigente", "% Estudiantes 2022-2023 sin matrícula vigente", "N° Estudiantes 2023 retirados en el año y sin matrícula vigente", "N° Estudiantes 2022 no matriculados en 2023", "Detalle N° Estudiantes 2021 no matriculados ni en 2022 ni 2023")
  
  
  colnames(xls_desvinc_ee_cur) <- c("cod_reg_rbd2022r2", "cod_deprov_rbd2022", "Nombre Deprov", "Comuna", "Nombre Sostenedor", "RUT Sostenedor", "email Sostenedor", "Teléfono Sostenedor", "Dependencia", "RBD", "Nombre RBD", "Último grado registrado", "N° Total Estudiantes sin matricula vigente",
  ...
  #colnames(xls_desvinc_ee_cur) <- c("cod_reg_rbd2022r2", "cod_deprov_rbd2022", "Nombre Deprov", "Comuna", "Nombre Sostenedor", "RUT Sostenedor", "email Sostenedor", "Teléfono Sostenedor", "Dependencia", "RBD", "Nombre RBD", "Grado", "Matrícula 2023 + estudiantes sin matrícula", "N° Estudiantes 2022-2023 sin matricula vigente", "% Estudiantes 2022-2023 sin matrícula vigente", "N° Estudiantes 2023 retirados en marzo y sin matrícula vigente", "N° Estudiantes 2022 no matriculados en 2023", "Detalle N° Estudiantes 2021 no matriculados ni en 2022 ni 2023")
  ...
  cods_reg <- unique(bd_asis$cod_reg_rbd2022r2)
  noms_deprov <- unique(bd_asis$nom_deprov_rbd)
  noms_deprov
  ...
  unique(bd_asis$cod_depe2)
  unique(bd$cod_depe2_2022r)

  cods_reg
  # resumen_rbd_ee <- resumen_rbd %>% filter(cod_deprov_rbd2022 == 11) 
  # colnames(resumen_rbd_ee)
  # resumen_rbd_ee <- resumen_rbd_ee %>% select(-c(cod_deprov_rbd2022))
  resumen_rbd
 
    
    {
    
       # cod_reg <- cods_reg[7]  # "Región de Los Ríos"
        # cod_reg
       for(cod_reg in cods_reg){     #c(42,53,101,136,134)
        try({
            tic()
            #**************************************************************************************************************************/
            ## 3.1 PRE RENDER  ----------------------------------------------------------------
            ...
            desvinc <- bd[bd$cod_reg_rbd2022r2 == cod_reg,]
            nrow(desvinc)
            data_plot2 <- data.frame(nom_grado = unique(desvinc$nom_grado))
    
            data_plot4 <- data_plot4_n %>% filter(nom_com_rbd_2022r %in% unique(desvinc$nom_com_rbd_2022r)) #data.frame(nom_rbd2022r = unique(desvinc$nom_rbd2022r))
            
            
            # n_mat2022 <- xls_desvinc_ee[xls_desvinc_ee$cod_reg_rbd2022r2 == cod_reg,]
            # n_mat2022 = sum(n_mat2022$`Matrícula 2022`, na.rm = T) # nrow(desvinc2)
            ...
            desvinc2 <- bd2[bd2$cod_reg_rbd_ret == cod_reg,]
            desvinc2 <- desvinc2 %>% filter(categoria_desert == "Retirado 2023")
            desvinc3 <- doble_desvinc[doble_desvinc$cod_reg_rbd == cod_reg,]
            ...
            ###PRUEBA
            n_desvinc <- xls_desvinc_ee[xls_desvinc_ee$cod_reg_rbd2022r2 == cod_reg,]
            n_desvinc = sum(n_desvinc$`N° Estudiantes 2022 no matriculados en 2023`, na.rm = T) # nrow(desvinc2)
  
            n_desvinc2 <- xls_desvinc_ee[xls_desvinc_ee$cod_reg_rbd2022r2 == cod_reg,]
            n_desvinc2 = sum(n_desvinc2$`N° Estudiantes 2023 retirados en el año y sin matrícula vigente`, na.rm = T) # nrow(desvinc2)
  
            n_desvinc3 <- xls_desvinc_ee[xls_desvinc_ee$cod_reg_rbd2022r2 == cod_reg,]
            n_desvinc3 = sum(n_desvinc3$`Detalle N° Estudiantes 2021 no matriculados ni en 2022 ni 2023`, na.rm = T) # nrow(desvinc2)
       
            ######
            # desvinc <- desvinc %>% select("run_alu", "nom_alu2022", "app_alu2022", "apm_alu2022", "gen_alu_2022r", "edad_alu_2022r", "nom_com_alu2022", "nom_grado", "prom_gral2022", "asistencia2022", "sit_fin_r_2022r", 
            #                               "rbd_2022r", "nom_rbd_2022r", "cod_depe2_2022r", "cod_reg_rbd2022r2", "cod_com_rbd_2022r", "nom_com_rbd_2022r", "cod_deprov_rbd2022", "nom_deprov_rbd2022", "nombre_sost_rbd2022", "rut_sost_rbd2022", "email_sost_rbd2022",  "tel_sost_rbd")
            # desvinc <- desvinc %>% arrange(nom_com_rbd_2022r, nom_grado)
            # 
            # 
            # 
            ...
            #                                "rbd_2022r", "nom_rbd_ret", "cod_depe2_ret", "cod_reg_rbd_ret", "cod_com_rbd_ret", "nom_com_rbd_ret", "cod_deprov_rbd_ret") 
            # desvinc2 <- desvinc2 %>% arrange(nom_com_rbd_ret, nom_grado_ret)
            # 
            
            ########## SE EXTRAEN LOS CASOS DE INASISTENCIA Y PARAMETROS 
            asis_crit <- bd_asis[bd_asis$cod_reg_rbd == cod_reg,]
            # n_mat_asis2023 = nrow(asis_crit)
            
            data_plot2_2 <- data.frame(nom_grado_ret = unique(desvinc2$nom_grado_ret))
            data_plot4_2 <- data_plot4_n_2 %>% filter(nom_com_rbd %in% unique(desvinc2$nom_com_rbd_ret)) #data.frame(nom_rbd2022r = unique(desvinc$nom_rbd2022r))
            
            data_plot3 <- data.frame(cod_curso = unique(asis_crit$cod_curso))
            data_plot3_n <- asis_crit %>% group_by(cod_curso) %>% summarise(n_total = n())
            data_plot6 <- data_plot6_n %>% filter(nom_com_rbd %in% unique(asis_crit$nom_com_rbd))
            ...
            n_mat2023 = xls_desvinc_ee[xls_desvinc_ee$cod_reg_rbd2022r2 == cod_reg,] #nrow(asis_crit)
            ...
            n_mat_asis2023 = xsl_asis_ee[xsl_asis_ee$cod_reg_rbd2022r2 == cod_reg,] #nrow(asis_crit)
            ...
            # 
            # n_mat2023_ret <- xls_desvinc_ee[xls_desvinc_ee$cod_reg_rbd2022r2 == cod_reg,]
            # n_mat2023_ret = sum(n_mat2023_ret$`Matrícula 2023 + Retirados 2023 sin matrícula actual`, na.rm = T) # nrow(desvinc2)
            ...
            #BD resumen EE
            resumen_asis_sost <- resumen_asis_ee %>% filter(cod_reg_rbd2022r2 == cod_reg)
            
            resumen_asis_sost <- resumen_asis_sost %>% select(nom_com_rbd, cod_depe2, cod_com_rbd, asis_promedio, asis_marzo, 
                                                              asis_abril, asis_mayo,
                                                              asis_junio, asis_julio, asis_agosto,
                                                              #asis_septiembre, asis_octubre, asis_noviembre, 
            ...
            print("hola")
            resumen_rbd_ee <- resumen_rbd %>% filter(cod_reg_rbd2022r2 == cod_reg)
            
            
            
            
            resumen_rbd_ee <- resumen_rbd_ee %>% select(-c("cod_reg_rbd2022r2"))
            # print("chao")
            
            print(resumen_asis_sost)
            print("-----")  
            resumen_desvinc_sost <- resumen_desvinc_ee %>% filter(cod_reg_rbd2022r2 == cod_reg & (n_desv > 0 | n_ret > 0))
            print(resumen_desvinc_sost)
            print("***")
            sum(resumen_desvinc_sost$n_total23, na.rm = T)
  
                      
            #Datos gráfico de barra de rangos de asistencia   #tagdiferencia --> listo. Se agrega la categoría sin información.
            ...
            n_inasis2023 = xsl_asis_ee[xsl_asis_ee$cod_reg_rbd2022r2 == cod_reg,] #nrow(asis_crit)
            n_inasis2023 = sum(n_inasis2023$`N° estudiantes con asistencia <85%`, na.rm = T)  #n_inasis2023 = nrow(asis_crit)
            #n_inasis2023 = nrow(asis_crit)
            
            
            asis_crit <- asis_crit %>% select("run_alu", "nom_alu", "app_alu", "apm_alu", "cod_depe2", "cod_curso", "rbd", "nom_rbd", "nom_com_rbd", "nom_reg_rbd_a", "porcentage_asistencia_acumulada", "inasistencia_grave_acumulada", "asistencia_categorias_acumulada", "tipo_asis_2")
            asis_crit <- asis_crit %>% arrange(nom_com_rbd, cod_curso, porcentage_asistencia_acumulada)
            nrow(asis_crit)
            ...
            # url_sin_fin_g = paste0("/Img temp/",cod_reg,"_sit_fin_g.png")
            # orca(p, url_sin_fin_g)
            
            url_sin_fin_g = paste0(getwd(),"/Img temp/",cod_reg,"_sit_fin_g.png")
          ...
          k <- plot_ly(x = ~data_plot2$nom_grado, y = ~data_plot2$n , type = "bar", marker = list(color = '#6bccd6'), text = data_plot2$n, texttemplate = '%{y}', textposition = 'outside') %>%
            layout(title="", xaxis=list(font = list(family = "verdana", color = '#007096', size = 22), title='<b>Grado 2022</b>'), yaxis=list(font = list(family = "verdana", color = '#007096', size = 22), title="<b>N° Estudiantes</b>", autotick = T), 
                   font=list(family = "verdana", color = '#007096', size = 22), plot_bgcolor='transparent', paper_bgcolor='transparent', autosize = F, width = 1400, height = 900, margin = list(l=-0.1, t = -0.1, r=-0.1))
          
          # url_cur_desv_g = paste0("/Img temp/", cod_reg,"_cur_desv_g.png")
          # orca(k, url_cur_desv_g)
          
          url_cur_desv_g = paste0(getwd(),"/Img temp/", cod_reg,"_cur_desv_g.png")
          save_image(k, url_cur_desv_g)          
          
          
          ########## GRAFICO BARRA CURSO RETIRADOS
          
          data_plot2_2 <- data_plot2_2 %>% arrange(nom_grado_ret)
          data_plot2_t_2 <- desvinc2 %>% group_by(nom_grado_ret) %>% summarise(n = n())
          data_plot2_2  <- left_join(data_plot2_2, data_plot2_t_2, by="nom_grado_ret")
          data_plot2_2  <- data_plot2_2 %>% mutate(n = ifelse(is.na(n), 0, n))
          
          data_plot2_2 <- data_plot2_2 %>% droplevels()
          data_plot2_2
          
          k2 <- plot_ly(x = ~data_plot2_2$nom_grado_ret, y = ~data_plot2_2$n , type = "bar", marker = list(color = '#6bccd6'), text = data_plot2_2$n, texttemplate = '%{y}', textposition = 'outside') %>%
            layout(title="", xaxis=list(font = list(family = "verdana", color = '#007096', size = 22), title='<b>Grado 2023</b>'), yaxis=list(font = list(family = "verdana", color = '#007096', size = 22), title="<b>N° Estudiantes</b>", autotick = T), 
                   font=list(family = "verdana", color = '#007096', size = 22), plot_bgcolor='transparent', paper_bgcolor='transparent', autosize = F, width = 1400, height = 900, margin = list(l=-0.1, t = -0.1, r=-0.1))
          
          # url_cur_desv_g_2 = paste0("/Img temp/", cod_reg,"_cur_desv_g_2.png")
          # orca(k2, url_cur_desv_g_2)
          
          url_cur_desv_g_2 = paste0(getwd(),"/Img temp/", cod_reg,"_cur_desv_g_2.png")
          ...
          j <- plot_ly(x=~data_plot3$cod_curso, y = ~data_plot3$porc_asiscrit , type = "bar", marker = list(color = '#6bccd6'), text = data_plot3$n, texttemplate = '%{y}', textposition = 'outside') %>%
            layout(title="", xaxis=list(font = list(family = "verdana", color = '#007096', size = 22), title='<b>Grado 2023</b>'), yaxis=list(font = list(family = "verdana", color = '#007096', size = 18), title="<b>% Estudiantes</b>", autotick = T, ticksuffix = "%"), 
                   font=list(family = "verdana", color = '#007096', size = 22), plot_bgcolor='transparent', paper_bgcolor='transparent', autosize = F, width = 1400, height = 900, margin = list(l=-0.1, t = -0.1, r=-0.1))
          
          # url_asis_crit_g = paste0("/Img temp/", cod_reg,"_asis_crit_g.png")
          # orca(j, url_asis_crit_g)
          
          url_asis_crit_g = paste0(getwd(),"/Img temp/", cod_reg,"_asis_crit_g.png")
          save_image(j, url_asis_crit_g) 
          
          ########## GRAFICO BARRA DESVINCULADOS POR Dependencia.
        
          data_plot4_t <- desvinc %>% group_by(nom_com_rbd_2022r) %>% summarise(n = n())
        
          print(data_plot4)
          print(data_plot4_t)
          data_plot4  <- left_join(data_plot4, data_plot4_t, by=c("nom_com_rbd_2022r"))
          ...
          m <- plot_ly() %>% add_trace(x = ~data_plot4$n, y = ~data_plot4$nom_com_rbd_2022r, marker = list(color = "#6bccd6"), type = "bar", orientation = "h", text = paste0("<b style='color:#007096'>",data_plot4$n,"</b>", "<i style='color:#007096';> (", data_plot4$porc_desvinc,"%)</i>"), textposition = "outside", textfont = list(family = "verdana", color = '#FFFFFF'))
          
          
          if(nrow(data_plot4) < 4){
            m <- m %>% layout(font = list(family = "verdana", color = '#007096', size = 17), title = "", bargap = 0.8,
                              xaxis = list(title="<b>N° Estudiantes 2022 no matriculados en 2023</b> <i>(% respecto a matrícula 2022)</i>", font = list(family = "verdana", size = 15), showlegend = FALSE, showgrid = FALSE, showticklabels = FALSE, side = "top", range = list(0, max(data_plot4$n)*1.2)),
                              yaxis = list(title="<b>Comuna</b>", font = list(family = "verdana", size = 17), showlegend = FALSE, categoryorder = "total ascending"),
                              plot_bgcolor='transparent', paper_bgcolor='transparent', autosize = F, width = 1200, height = 800, margin = list(l=-0.1, t = -0.1, r=-0.1)
            )
          } else if(nrow(data_plot4) > 35){
            m <- m %>% layout(font = list(family = "verdana", color = '#007096', size = 17), title = "", 
                              xaxis = list(title="<b>N° Estudiantes 2022 no matriculados en 2023</b> <i>(% respecto a matrícula 2022)</i>", font = list(family = "verdana", size = 15), showlegend = FALSE, showgrid = FALSE, showticklabels = FALSE, side = "top", range = list(0, max(data_plot4$n)*1.2)),
                              yaxis = list(title="<b>Comuna</b>", font = list(family = "verdana", size = 17), showlegend = FALSE, categoryorder = "total ascending"),
                              plot_bgcolor='transparent', paper_bgcolor='transparent', autosize = F, width = 1200, height = 1600, margin = list(l=-0.1, t = -0.1, r=-0.1)
            )  
          } else{
            m <- m %>% layout(font = list(family = "verdana", color = '#007096', size = 17), title = "", 
                              xaxis = list(title="<b>N° Estudiantes 2022 no matriculados en 2023</b> <i>(% respecto a matrícula 2022)</i>", font = list(family = "verdana", size = 15), showlegend = FALSE, showgrid = FALSE, showticklabels = FALSE, side = "top", range = list(0, max(data_plot4$n)*1.2)),
          ...
          # url_sost_desv_g = paste0("/Img temp/", cod_reg,"_sost_desv_g.png")
          # orca(m, url_sost_desv_g)
          
          url_sost_desv_g = paste0(getwd(),"/Img temp/", cod_reg,"_sost_desv_g.png")
          save_image(m, url_sost_desv_g) 
          
          ########## GRAFICO BARRA RETIRADOS POR DEPENDENCIA.
          table(desvinc2$cod_reg_rbd_ret)    #  nom_com_rbd_ret)
          table(desvinc2$cod_reg_rbd_ret)
          colnames(desvinc2)
          
          sum(data_plot4_2$n_ret)
          
          data_plot4_t_2 <- desvinc2 %>% group_by(nom_com_rbd_ret) %>% summarise(n = n())
          data_plot4_2
          data_plot4_2  <- left_join(data_plot4_2, data_plot4_t_2, by=c("nom_com_rbd"="nom_com_rbd_ret"))
          data_plot4_2
          ...
          m2 <- plot_ly() %>% add_trace(x = ~data_plot4_2$n, y = ~data_plot4_2$nom_com_rbd, marker = list(color = "#6bccd6"), type = "bar", orientation = "h", text = paste0("<b style='color:#007096'>",data_plot4_2$n,"</b>", "<i style='color:#007096';> (", data_plot4_2$porc_desvinc,"%)</i>"), textposition = "outside", textfont = list(family = "verdana", color = '#FFFFFF'))
          
          
          if(nrow(data_plot4_2) < 4){
            m2 <- m2 %>% layout(font = list(family = "verdana", color = '#007096', size = 17), title = "", bargap = 0.8,
                              xaxis = list(title="<b>N° Estudiantes retirados durante 2023 y sin matrícula actual</b> <i>(% respecto a matrícula 2023)</i>", font = list(family = "verdana", size = 15), showlegend = FALSE, showgrid = FALSE, showticklabels = FALSE, side = "top", range = list(0, max(data_plot4_2$n)*1.2)),
                              yaxis = list(title="<b>Comuna</b>", font = list(family = "verdana", size = 17), showlegend = FALSE, categoryorder = "total ascending"),
                              plot_bgcolor='transparent', paper_bgcolor='transparent', autosize = F, width = 1200, height = 800, margin = list(l=-0.1, t = -0.1, r=-0.1)
            )
          } else if(nrow(data_plot4_2) > 35){
            m2 <- m2 %>% layout(font = list(family = "verdana", color = '#007096', size = 17), title = "", 
                                xaxis = list(title="<b><b>N° Estudiantes retirados durante 2023 y sin matrícula actual</b> <i>(% respecto a matrícula 2023)</i>", font = list(family = "verdana", size = 15), showlegend = FALSE, showgrid = FALSE, showticklabels = FALSE, side = "top", range = list(0, max(data_plot4_2$n)*1.2)),
                                yaxis = list(title="<b>Comuna</b>", font = list(family = "verdana", size = 17), showlegend = FALSE, categoryorder = "total ascending"),
                                plot_bgcolor='transparent', paper_bgcolor='transparent', autosize = F, width = 1200, height = 1600, margin = list(l=-0.1, t = -0.1, r=-0.1)
            )  
            } else{
          ...
          # url_sost_desv_g_2 = paste0("/Img temp/", cod_reg,"_sost_desv_g_2.png")
          # orca(m2, url_sost_desv_g_2)
          
          url_sost_desv_g_2 = paste0(getwd(),"/Img temp/", cod_reg,"_sost_desv_g_2.png")
          ...
          # url_sost_asist_g = paste0("/Img temp/", cod_reg,"_sost_asist_g.png")
          # orca(o, url_sost_asist_g)

          url_sost_asist_g = paste0(getwd(),"/Img temp/", cod_reg,"_sost_asist_g.png")
          save_image(o, url_sost_asist_g)         
          
          ########## GRAFICO BARRA ASIS CRIT POR EE DEL SOSTENEDOR.
          
          data_plot6_t <- asis_crit %>% group_by(nom_com_rbd) %>% summarise(n = n()) %>% ungroup()
          data_plot6  <- left_join(data_plot6, data_plot6_t, by=c("nom_com_rbd"))
          ...
          q <- plot_ly() %>% add_trace(x = ~data_plot6$porc_asiscrit, y = ~data_plot6$nom_com_rbd,  marker = list(color = "#6bccd6"), type = "bar", orientation = "h", text = paste0("<b style='color:#007096'>", data_plot6$porc_asiscrit, "%","</b>", "<i style='color:#007096';> (", data_plot6$n," de ", data_plot6$n_total,")</i>"), textposition = "outside", textfont = list(family = "verdana", color = '#FFFFFF'))
          
          if(nrow(data_plot6) < 4){  #Si son pocos EE se achican las barras
            q <- q %>% layout(font = list(family = "verdana", color = '#007096', size = 17), title = "", bargap = 0.8,
                              ...
                              xaxis = list(title="<b>% Estudiantes 2023 con asistencia bajo 85%</b> <i> (N° Estudiantes respecto a matrícula 2023)</i>", font = list(family = "verdana", size = 15), range = list(0, max(data_plot6$porc_asiscrit) + 22), showlegend = FALSE, ticksuffix = "%", showgrid = FALSE, side = "top"),
                              yaxis = list(title="<b>Comuna</b>", font = list(family = "verdana", size = 17), showlegend = FALSE, categoryorder = "total ascending"),
                              plot_bgcolor='transparent', paper_bgcolor='transparent', autosize = F, width = 1200, height = 800, margin = list(l=-0.1, t = -0.1, r=-0.1)
            )
          } else if(nrow(data_plot6) > 35){
            q <- q %>% layout(font = list(family = "verdana", color = '#007096', size = 17), title = "",
                              #xaxis2 = list(title="<b>% Estudiantes con asistencia grave 2023</b>", font = list(family = "verdana", size = 8, color = "#e94860"), side = "top", 
                              #             overlaying = "x", position=1,  range = list(0, max(data_plot6$porc_asiscrit) + 10), showgrid = FALSE, zeroline = FALSE, 
                              #            tickfont = list(color = "#e94860"), ticksuffix = "%", titlefont = list(color = "#e94860")),
                              xaxis = list(title="<b>% Estudiantes 2023 con asistencia bajo 85%</b> <i> (N° Estudiantes respecto a matrícula 2023)</i>", font = list(family = "verdana", size = 15), range = list(0,  max(data_plot6$porc_asiscrit) + 22), showlegend = FALSE, ticksuffix = "%", showgrid = FALSE, side = "top"),
          ...
          # url_sost_asisgrave_g = paste0("/Img temp/", cod_reg,"_sost_asisgrave_g.png")
          # orca(q, url_sost_asisgrave_g)
          
          url_sost_asisgrave_g = paste0(getwd(),"/Img temp/", cod_reg,"_sost_asisgrave_g.png")
          ...
          rmarkdown::render("Codigos/seremi/RMKD Informe Seremi.Rmd",
                            params = list("cod_deprov" = cod_reg, "nom_deprov" = nom_deprov, "desvinc" = desvinc, "desvinc2" = desvinc2, "desvinc3" = desvinc3, "asis_crit" = asis_crit, "url_sin_fin_g" = url_sin_fin_g, "url_cur_desv_g" = url_cur_desv_g, "url_asis_crit_g" = url_asis_crit_g, "url_sost_desv_g" = url_sost_desv_g, "url_sost_desv_g_2" = url_sost_desv_g_2, "url_cur_desv_g_2" = url_cur_desv_g_2, "url_sost_asist_g" = url_sost_asist_g,
                                          "url_sost_asisgrave_g" = url_sost_asisgrave_g, "n_desvinc" = n_desvinc, "n_desvinc2" = n_desvinc2, "n_desvinc3" = n_desvinc3, "n_mat2023" = n_mat2023, "n_mat_asis2023" = n_mat_asis2023, "n_inasis2023" = n_inasis2023, "resumen_asis_sost" = resumen_asis_sost, "resumen_desvinc_sost" = resumen_desvinc_sost, "resumen_rbd" = resumen_rbd_ee),
                            output_file = paste0("Outputs/seremi/", nom_deprov, ".pdf"))
          print(paste0("Listo ", i ," de ", nn, " Seremi. (Codigo: ", cod_reg, ")"))
      ...
      xsl_asis_ee_deprov <- xsl_asis_ee %>% ungroup() %>% filter(cod_reg_rbd2022r2 == cod_reg) %>% select(-c("cod_reg_rbd2022r2", "cod_deprov_rbd"))
      xsl_asis_ee_cur_deprov <- xsl_asis_ee_cur %>% ungroup() %>% filter(cod_reg_rbd2022r2 == cod_reg) %>% select(-c("cod_reg_rbd2022r2", "cod_deprov_rbd"))
      xls_desvinc_ee_deprov <- xls_desvinc_ee %>% ungroup() %>% filter(cod_reg_rbd2022r2 == cod_reg) %>% select(-c("cod_reg_rbd2022r2", "cod_deprov_rbd2022"))
      xls_desvinc_ee_cur_deprov <- xls_desvinc_ee_cur %>% ungroup() %>% filter(cod_reg_rbd2022r2 == cod_reg) %>% select(-c("cod_reg_rbd2022r2", "cod_deprov_rbd2022"))
      
  
      wb <- createWorkbook()
      addWorksheet(wb, sheetName = "Resumen no_matric x RBD")
      addWorksheet(wb, sheetName = "Detalle no_matric x RBD y grado")
      addWorksheet(wb, sheetName = "Resumen asist x RBD")
      addWorksheet(wb, sheetName = "Detalle asist x RBD y grado")
  
      writeData(wb, "Resumen no_matric x RBD", xls_desvinc_ee_deprov)
      writeData(wb, "Detalle no_matric x RBD y grado", xls_desvinc_ee_cur_deprov)
      writeData(wb, "Resumen asist x RBD", xsl_asis_ee_deprov)
      writeData(wb, "Detalle asist x RBD y grado", xsl_asis_ee_cur_deprov)
  
      pct = createStyle(numFmt="0.0%")
      addStyle(wb, "Resumen no_matric x RBD", style=pct, cols=c(12), rows=2:(nrow(xls_desvinc_ee_deprov)+1), gridExpand=TRUE)
      # addStyle(wb, "Detalle no_matric x RBD y grado", style=pct, cols=c(13), rows=2:(nrow(xls_desvinc_ee_cur_deprov)+1), gridExpand=TRUE)
      addStyle(wb, "Resumen asist x RBD", style=pct, cols=c(10:13, 16, 22:27), rows=2:(nrow(xsl_asis_ee_deprov)+1), gridExpand=TRUE)
      addStyle(wb, "Detalle asist x RBD y grado", style=pct, cols=c(11:14, 17, 23:28), rows=2:(nrow(xsl_asis_ee_cur_deprov)+1), gridExpand=TRUE)
      
      wb
      saveWorkbook(wb, paste0("Outputs/seremi/", nom_deprov, ".xlsx"), overwrite = T)
