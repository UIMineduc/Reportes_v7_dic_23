...
\vspace{12mm}
\textbf{\textcolor{celeste_gob}{\Large{Secretaría Regional de Educación}}}\\
\vspace{12mm}
...
\textcolor{celeste_gob}{\vspace{-7mm}\section{Secretarías Regionales de Educación:\\}}
\vspace{10mm}
```{=tex}
...
\\
La estructura del presente informe comienza con la descripción de su contenido, un resumen de la información regional de estudiantes sin matrícula o retirados en 2023, además de un resumen de estudiantes con baja asistencia durante el mismo periodo. Para su elaboración, se ha utilizado la información actualizada al 31 de agosto del presente año.\\
\\
...
\textcolor{azul_gob}{\normalsize{Este reporte busca entregar información a los equipos de las Secretarías Regionales de Educación respecto a distintas situaciones de trayectoria educativa interrumpida o irregular de estudiantes que están actualmente en su territorio, o para quienes algún centro educativo de su región es el último registro del que se tiene. En este documento podrá encontrar un resumen agregado a nivel de región y gráficos con el detalle por comuna y grado. Además, se presentan tablas con el detalle por establecimiento con información agregada de estudiantes que se encuentran en cada una de las situaciones de trayectoria educativa interrumpida o irregular.\\}} 
...
\textcolor{azul_gob}{\normalsize{\\Como se observa en el esquema anterior, el reporte se divide en dos grandes secciones. La primera es la sección que resume datos de estudiantes sin matrícula vigente, que se enfoca en presentar información a nivel agregada de estudiantes que tienen como último registro de matrícula algún establecimiento de su región entre los años 2021 y 2023, pero que actualmente no se encuentran matriculados en ningún establecimiento. La segunda sección es la de estudiantes con baja asistencia, que entrega información de asistencia de estudiantes que están matriculados en algún establecimiento de su región, centrándose en aquellos con un promedio de asistencia menor a un 85\%.}}
...
\textcolor{azul_gob}{\textbf{\normalsize{1.1 Estudiantes 2023 retirados dentro del año y que están sin matrícula vigente:}} Este grupo representa a estudiantes que, habiendo estado matriculados en algún establecimiento de su región al inicio del \textbf{año 2023}, se encuentran actualmente en situación de "Retirado" y no están matriculados en ningún otro establecimiento al 31 de agosto de 2023.\\}

\vspace{1mm}

\textcolor{azul_gob}{\textbf{\normalsize{1.2 Estudiantes 2022 que no se han matriculado en todo el 2023:}} Estudiantes que, estando matriculados el \textbf{año 2022} en algún establecimiento de su región, no retornaron al año escolar 2023 y no se han matriculado en ningún otro establecimiento.\\}


\vspace{1mm}


\textcolor{azul_gob}{\textbf{\normalsize{1.3 Estudiantes 2021 que no se han matriculado ni en 2022 ni 2023:}} Estudiantes que, habiendo estado matriculados en algún establecimiento de su región en el \textbf{año 2021}, no se han matriculado ni en el año 2022 ni 2023 en ningún otro establecimiento.\\}
...
\vspace{0mm}
\begin{flushleft}
  \normalsize{\textcolor{azul_gob}{En este reporte, además de estudiantes del año 2022 y 2023 sin matrícula vigente, se presenta la información de estudiantes que estaban en algún establecimiento de su región el año 2021 y no se han matriculado en todo el 2022 ni 2023 en ningún otro establecimiento. Estos casos se resumen en el siguiente cuadro:}}\\
\end{flushleft} 
...
\textcolor{azul_gob}{\normalsize{\textbf{\textcolor{rojo_gob}{1.2 Estudiantes 2022 que no se han matriculado en todo el 2023:}} \\ \\ \small{Resumen de estudiantes que, estando matriculados el año 2022 en algún establecimiento de su región, no han estado matriculados en ningún otro establecimiento hasta el 31 de agosto 2023 (no incluye a quienes egresaron de 4° medio el año anterior).}}}
...
pander("\\newpage
\\textcolor{rojo_gob}{\\large{\\textbf{\\\\Cantidad de estudiantes sin matrícula vigente con detalle de su situación de trayectoria educativa interrumpida o irregular, según comuna y dependencia.\\\\}}}
\\renewcommand{\\arraystretch}{1}")
}
```
...
if(nrow(resumen_desvinc_sost) > 0 ) {
  ...
  resumen_desvinc_sost <- resumen_desvinc_sost %>% select("nom_com_rbd_2022r", "cod_depe2_2022r", "n_mat_teo_total23", "desv_total23", "porc_total23", "n_ret","n_desv", "n_dobledesv")
...
pander("\\begin{flushleft}
       \\textit{\\textcolor{azul_gob}{\\footnotesize{\\textbf{Gráfico 2.1:} Distribución \\textit{(y número)} de estudiantes de la región según su rango de asistencia 2023:}}}
       \\end{flushleft}")
...
pander("\\begin{flushleft}
     \\textit{\\textcolor{azul_gob}{\\footnotesize{\\textbf{Gráfico 2.2:} Porcentaje de estudiantes con asistencia bajo 85\\%, por comuna:}}}
     \\end{flushleft}")
#}
```
\begin{center}
...
if(nrow(resumen_sost) > 0 ) {
  ...
  kbl1 <- kable(resumen_sost, "latex", booktabs = T, longtable = T,  align = c("l","l",'c','c','c','c','c','c','c','c','c','c'), caption = "\\textcolor{azul_gob}{\\textbf{Tabla 2.1:} Detalle de asistencia 2023 por comuna y dependencia.}", linesep = "")
   ...
   kbl1 %>%
     ...
     column_spec(column = c(3:8), width = "10em") %>%
     column_spec(column = c(9), bold = T, color = "#0f69b4", width = "10em") %>%
     column_spec(column = c(10:11), width = "10em") %>%
     column_spec(column = c(12), width = "11em", bold = T, color = "#0f69b4") %>%
...
\newpage
\textcolor{rojo_gob}{\large{\textbf{\\2.2 Datos de rangos de asistencia de estudiantes de las comunas de la región.\textcolor{azul_gob}{\footnote{\textcolor{azul_gob}{No se considera la asistencia de establecimientos particulares pagados. Si otro tipo de dependencia no aparece para alguna comuna es porque no hay registrados establecimientos de esa dependencia en ese territorio.}}}\\}}}
\renewcommand{\arraystretch}{1.1}
...
kbl1 <- kable(resumen_sost, "latex", booktabs = T, longtable = T,  align = c("l","l",'c','c','c','c','c'), caption = "\\textcolor{azul_gob}{\\textbf{Tabla 2.2:} Porcentaje de estudiantes por rangos de asistencia por comuna y dependencia.}", linesep = "")
 ...
 kbl1 %>% kable_minimal(c("hover")) %>%
   kable_styling(font_size = 6, latex_options=c("scale_down", "HOLD_position", "repeat_header"), repeat_header_text = "\\textcolor{azul_gob}{\\textit{(continuación)}}", full_width = T, position = "left", html_font = "Verdana") %>% column_spec(column = 1:7, color = "#004677") %>% row_spec(0, bold = T, color = "#004677") %>%
   column_spec(column = c(1), width = "15em", bold = T) %>%
...
<!-- \newpage -->
<!-- \textcolor{rojo_gob}{\large{\textbf{\\ANEXO 1: Resumen de estudiantes no matriculados y con asistencia crítica por establecimiento.\\}}} -->
<!-- \begin{turnpage} -->
\begin{landscape} 
\newpage
\begin{center}
...
for(comuna in unique(resumen_rbd_mkd$Comuna)[1]){
 ...
 kbl1 <- kbl1 %>%
   ...
   column_spec(column = c(1), width = "10em", bold = T) %>%
   ...
   add_header_above(c("","","", "Resumen asistencia 2023" = 10, "Resumen estudiantes sin matrícula vigente" = 3, "Detalle N° estudiantes sin matrícula vigente" = 3), color = "#004677") %>% row_spec(0, bold = T, color = "#004677") %>% collapse_rows(columns = 1, valign = "middle", latex_hline = "major")  # %>% gsub("\\\\\\\\($|\n)", "\\\\\\\\\\\\nopagebreak\\1", .) %>% gsub("(\\\\cmidrule[{][^}]*[}])", "\\1\\\\pagebreak[0]", .)
