...
resumen_asis_ee <- bd_asis %>% filter(cod_depe2 != 3) %>% filter(!is.na(inasistencia_grave_acumulada)) %>% group_by(cod_reg_rbd2022r2, nom_com_rbd, cod_com_rbd, cod_depe2) %>% summarise(asis_marzo= mean(porcentage_asistencia_marzo, na.rm = TRUE), 
  ...
  resumen_rbd
    ...
    
    {
       ...
       # cod_reg <- cods_reg[7]  # "Región de Los Ríos"
        # cod_reg
       for(cod_reg in cods_reg[7]){     #c(42,53,101,136,134)
        try({
            tic()
            #**************************************************************************************************************************/
            ## 3.1 PRE RENDER  ----------------------------------------------------------------
          ...
          rmarkdown::render("Codigos/seremi/RMKD Informe Seremi (EPJA).Rmd",
                            ...
                            output_file = paste0("Outputs/Seremi (EPJA)/", nom_deprov, " (EPJA).pdf"))
      ...
      addStyle(wb, "Resumen asist x RBD", style=pct, cols=c(10:16, 19, 25:29), rows=2:(nrow(xsl_asis_ee_deprov)+1), gridExpand=TRUE)
      addStyle(wb, "Detalle asist x RBD y grado", style=pct, cols=c(11:17, 20, 26:30), rows=2:(nrow(xsl_asis_ee_cur_deprov)+1), gridExpand=TRUE)
      
      wb
      saveWorkbook(wb, paste0("Outputs/Seremi (EPJA)/", nom_deprov, " (EPJA).xlsx"), overwrite = T)
