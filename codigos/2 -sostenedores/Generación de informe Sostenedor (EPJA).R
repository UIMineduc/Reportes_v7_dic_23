...
#Búsqueda pendientes
{
  ...
  df <- data.frame(rutsost = rutsost1)
  archivos <- list.files("Outputs/Sostenedores (EPJA)")
  ...
  pdfs <- pdfs %>% mutate(rutsost  = gsub("-EPJA.pdf", "", nombre_pdf)) %>% mutate(rutsost  = as.numeric(rutsost))
  head(pdfs)
  nrow(pdfs)
  head(pdfs)
  
  df <- left_join(df, pdfs, by = "rutsost")
  pendientes <- df %>% filter(is.na(nombre_pdf))
  pendientes <- unique(pendientes$rutsost)
  pendientes
...
#######
#rut_sost = 62000660
for(rut_sost in pendientes){     #rutsost1 #c(42,53,101,136,134)
  try({
    tic()
    #**************************************************************************************************************************/
    ## 3.1 PRE RENDER  ----------------------------------------------------------------
    ...
    # ## 3.2 RENDER  ----------------------------------------------------------------
    # #**************************************************************************************************************************/
    rmarkdown::render("Codigos/sostenedores/RMKD Informe Sostenedor (EPJA).Rmd",
                      ...
                      output_file = paste0("Outputs/Sostenedores (EPJA)/", rut_sost, "-EPJA.pdf"))
    ...
    saveWorkbook(wb, paste0("Outputs/Sostenedores (EPJA)/", rut_sost, "-EPJA.xlsx"), overwrite = T)
