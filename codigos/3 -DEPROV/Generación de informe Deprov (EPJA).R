...
#Se agrega nombre grado a los retirados   #tagnuevo (1)  --> listo
...
colnames(bd_desvinc_intran)
 ...
 # bd_desvinc_intran_r2 <- bd_desvinc_intran %>% select("run_alu", "nom_alu", "app_alu", "apm_alu", "gen_alu", "edad_alu", "nom_com_alu", "cod_grado", "cod_ense", "fec_ret_rbd", "run_alu2",
 #                                                      "rbd", "nom_rbd", "cod_depe2", "cod_reg_rbd", "cod_com_rbd", "nom_com_rbd", "cod_deprov_rbd") 
 bd_desvinc_intran_r2 <- bd_desvinc_intran %>% select("run_alu", "nom_alu", "app_alu", "apm_alu", "gen_alu", "edad_alu", "nom_com_alu", "cod_grado", "cod_ense", "nom_grado", "fec_ret_rbd", "run_alu2",
                                                      "rbd", "nom_rbd", "cod_depe2", "cod_reg_rbd", "cod_com_rbd", "nom_com_rbd", "cod_deprov_rbd", "nom_deprov_rbd", "nom_reg_rbd_a") #tagnuevo se agregan campos de retirados
 ...
 # deprov_sost <- bd %>% group_by(nom_deprov_rbd2022, cod_deprov_rbd2022, rbd_2022r) %>% summarise(n = n())
 # deprov_sost <- bd %>% group_by(nom_deprov_rbd2022, cod_deprov_rbd2022, rbd_2022r) %>% summarise(n = n())
 ...
 sin_datos <- bd_asis %>% filter(is.na(nom_deprov_rbd))
 ...
 data_plot4_n <- bd %>% group_by(cod_com_rbd_2022r, nom_com_rbd_2022r, cod_depe2_2022r) %>% summarise(n_total = n()) %>% ungroup()  #**
 data_plot4_n_2 <- bd_asis %>% group_by(cod_com_rbd, nom_com_rbd, cod_depe2) %>% summarise(n_total = n()) %>% ungroup()   #**
 bd_asis <- bd_asis %>% filter(cod_depe2 != 3)
 data_plot6_n <- bd_asis %>% group_by(cod_com_rbd, nom_com_rbd, cod_depe2) %>% summarise(n_total = n()) %>% ungroup()   #**
 ...
 resumen_asis_ee <- bd_asis %>% filter(cod_depe2 != 3) %>% filter(!is.na(inasistencia_grave_acumulada)) %>% group_by(cod_deprov_rbd, nom_com_rbd, cod_com_rbd, cod_depe2) %>% summarise(asis_marzo= mean(porcentage_asistencia_marzo, na.rm = TRUE), 
 ...
 resumen_asis_ee2 <- bd_asis %>% filter(cod_depe2 != 3) %>% group_by(cod_deprov_rbd, nom_com_rbd, cod_com_rbd, cod_depe2, asistencia_categorias_acumulada) %>% summarise(n_cat = n())
 ...
 resumen_asis_ee = left_join(resumen_asis_ee, resumen_asis_ee2, by = c("cod_deprov_rbd", "nom_com_rbd", "cod_com_rbd", "cod_depe2"))
 
 
 #tagnuevo (1)
 resumen_asis_ee_aju = left_join(resumen_asis_ee_aju, resumen_asis_ee2, by = c("cod_deprov_rbd", "nom_com_rbd", "cod_com_rbd", "cod_depe2"))
 ...
 #resumen_desvinc_ee <- bd  %>%  group_by(cod_deprov_rbd2022, nom_com_rbd_2022r, cod_com_rbd_2022r, nom_grado) %>% summarise(n_total = n(), n_ret = sum(retirados))#summarise(n_total = n(), n = sum(desert_glob_total_aju_marzo_2023))
 
 mat_2023_com_cur <- bd_asis_todos %>% group_by(cod_com_rbd, nom_grado) %>% summarise(n_total23 = n()) %>% ungroup()
 mat_2023_com_cur <- mat_2023_com_cur %>% rename(cod_com_rbd_2022r = cod_com_rbd)
 
 resumen_desvinc_ee <- bd %>% group_by(cod_deprov_rbd2022, nom_com_rbd_2022r, cod_com_rbd_2022r, nom_grado) %>% summarise(n_total = n(), n_desv = sum(desvinculados, na.rm = T))#summarise(n_total = n(), n = sum(desert_glob_total_aju_marzo_2023))
 resumen_desvinc_ee2 <- bd2 %>% group_by(cod_deprov_rbd_ret, nom_com_rbd_ret, cod_com_rbd_ret, nom_grado_ret) %>% summarise(n_ret = sum(retirados, na.rm = T)) %>% ungroup() #%>% select(cod_deprov_rbd_ret,nom_com_rbd_ret, cod_com_rbd_ret, nom_grado_ret, n_ret) #summarise(n_total = n(), n = sum(desert_glob_total_aju_marzo_2023))
 resumen_dobledesvinc_ee <- doble_desvinc %>% group_by(cod_com_rbd, nom_grado) %>% summarise(n_dobledesv = n()) %>% ungroup() #%>% select(rbd, n_dobledesv)#summarise(n_total = n(), n = sum(desert_glob_total_aju_marzo_2023))

 
 resumen_desvinc_ee <- full_join(mat_2023_com_cur, resumen_desvinc_ee, by = c("cod_com_rbd_2022r" = "cod_com_rbd_2022r" , "nom_grado" = "nom_grado"))
 resumen_desvinc_ee <- full_join(resumen_desvinc_ee, resumen_desvinc_ee2, by = c("cod_deprov_rbd2022" = "cod_deprov_rbd_ret", "nom_com_rbd_2022r" = "nom_com_rbd_ret", "cod_com_rbd_2022r" = "cod_com_rbd_ret" , "nom_grado" = "nom_grado_ret"))
 resumen_desvinc_ee <- left_join(resumen_desvinc_ee, resumen_dobledesvinc_ee, by = c("cod_com_rbd_2022r" = "cod_com_rbd" , "nom_grado" = "nom_grado"))
 ...
 resumen_desvinc_ee <- resumen_desvinc_ee %>% mutate(desv_total23 = n_ret + n_desv + n_dobledesv)
 ...
 resumen_desvinc_ee <- resumen_desvinc_ee %>% arrange(nom_com_rbd_2022r, nom_grado)
 ...
 xls_desvinc_ee_cur <- xls_desvinc_ee_cur %>% arrange(n_desv)
 ...
 # #colnames(bd_asis)
 # datos_rbd2023 <- bd_asis %>% group_by(rbd) %>% slice(1) %>% select(rbd, cod_depe2, nom_rbd, cod_deprov_rbd, nom_deprov_rbd, cod_com_rbd, nom_com_rbd, cod_reg_rbd, nom_reg_rbd_a, nombre_sost_rbd2022, rut_sost_rbd2022, email_sost_rbd2022, tel_sost_rbd)
 # datos_rbd2022 <- bd %>% group_by(rbd_2022r) %>% slice(1) %>% select(rbd_2022r, nom_deprov_rbd2022, cod_deprov_rbd2022, cod_depe2_2022r, nom_rbd_2022r, cod_com_rbd_2022r, nom_com_rbd_2022r, cod_reg_rbd2022r2, nombre_sost_rbd2022, rut_sost_rbd2022, email_sost_rbd2022, tel_sost_rbd)
 # datos_rbd2023_ret <- bd2 %>% group_by(rbd_ret) %>% slice(1) %>% select(rbd_ret, cod_depe2_ret, nom_rbd_ret, cod_deprov_rbd_ret, nom_deprov_rbd_ret, cod_com_rbd_ret, nom_com_rbd_ret, cod_reg_rbd_ret, nom_reg_rbd_a_ret, nombre_sost_rbd2022_ret, rut_sost_rbd2022_ret, email_sost_rbd2022_ret, tel_sost_rbd_ret)
 # 
 colnames(bd)
 ...
 # xls_desvinc_ee2 <- bd2  %>%  group_by(cod_deprov_rbd_ret, rbd_ret) %>% summarise(n_ret = sum(retirados, na.rm = T)) %>% ungroup() %>% select(cod_deprov_rbd_ret, rbd_ret, n_ret)
 ...
 # xls_desvinc_ee <- left_join(xls_desvinc_ee, datos_rbd2022, by = c("rbd_2022r"))
 # xls_desvinc_ee <- left_join(xls_desvinc_ee, datos_rbd2023_ret, by = c("rbd_2022r" = "rbd_ret"))
 # xls_desvinc_ee <- xls_desvinc_ee %>% arrange(nom_rbd_2022r)
 # 
 ...
 #ACA(3)
#Se limpian los datos nulos pasandolos a 0
 ...
 xsl_asis_ee_cur <- xsl_asis_ee_cur %>% mutate(sin_info = ifelse(!is.na(`NA`), round(`NA`/n_total2, 3), 0))      #ifelse(!is.na(`5`), paste0(100*round(`5`/n_total2, 3),"%"), "0%"))
 ...
 xsl_asis_ee <- xsl_asis_ee %>% mutate(sin_info = ifelse(!is.na(`NA`), round(`NA`/n_total2, 3), 0))      #ifelse(!is.na(`5`), paste0(100*round(`5`/n_total2, 3),"%"), "0%"))
 ...
 xls_desvinc_ee_cur<- xls_desvinc_ee_cur %>% select(cod_reg_rbd2022r2, cod_deprov_rbd, nom_deprov_rbd, nom_com_rbd, nombre_sost_rbd2022, rut_sost_rbd2022, email_sost_rbd2022, 
                                                    ...
                                                    #tel_sost_rbd, cod_depe2_2022r, rbd_2022r, nom_rbd, nom_grado, n_mat_teo_total23, desv_total23, porc_total23,  n_ret, n_desv, n_dobledesv)
 ...
 #tbl_desvinc_ee <-tbl_desvinc_ee %>% rename(n_descinv = desv_total23, n_total_desvinc = n_mat_teo_total2)
 
 tbl_asis_ee <- xsl_asis_ee %>% select(cod_deprov_rbd, nom_com_rbd, cod_depe2, rbd, nom_rbd, nombre_sost_rbd2022, asis_marzo, 
                                       asis_abril, asis_mayo,
                                       asis_junio, asis_julio, asis_agosto,
                                       #asis_septiembre, asis_octubre, asis_noviembre,
 ...
 colnames(resumen_rbd) <- c("cod_deprov_rbd", "Comuna", "Dependencia", "RBD", "Nombre establecimiento", "Nombre Sostenedor", "Asist. promedio marzo", 
                            ...
                            "Asist. promedio acumulada", "Matrícula 2023", "N° estud. con asistencia <85%", "% estud. con asistencia <85%", "Matrícula 2023 + estud. sin matrícula", "N° Estud. 2022-2023 sin matricula vigente", "% Estud. 2022-2023 sin matricula vigente", "N° Estudiantes 2023 retirados en el año y sin matrícula vigente", "N° Estudiantes 2022 no matriculados en 2023", "Detalle N° Estudiantes 2021 no matriculados ni en 2022 ni 2023")
 
 
 
 colnames(xsl_asis_ee_cur) <- c("Región abreviada", "cod_deprov_rbd", "Nombre Deprov", "Comuna", "Nombre Sostenedor", "RUT Sostenedor", 
                                "email Sostenedor", "Teléfono Sostenedor", "Dependencia", "RBD", "Nombre RBD", "Grado", "Asistencia promedio marzo",
 ...
 colnames(xsl_asis_ee) <- c("Región abreviada", "cod_deprov_rbd", "Nombre Deprov", "Comuna", "Nombre Sostenedor", "RUT Sostenedor", 
                            "email Sostenedor", "Teléfono Sostenedor", "Dependencia", "RBD", "Nombre RBD", "Asistencia promedio marzo",
                            "Asistencia promedio abril", "Asistencia promedio mayo",
                            "Asistencia promedio junio","Asistencia promedio julio","Asistencia promedio agosto",
                            #"Asistencia promedio sept.", "Asistencia promedio octubre", "Asistencia promedio nov.", 
                            "Asistencia promedio acumulada", "N° Estudiantes totales",
 ...
 colnames(xls_desvinc_ee) <- c("Región", "cod_deprov_rbd2022", "Nombre Deprov", "Comuna", "Nombre Sostenedor", "RUT Sostenedor", "email Sostenedor", 
                               "Teléfono Sostenedor", "Dependencia", "RBD", "Nombre RBD", "Matrícula 2023 + estudiantes sin matrícula", "N° Estudiantes 2022-2023 sin matricula vigente", "% Estudiantes 2022-2023 sin matricula vigente", 
                               "N° Estudiantes 2023 retirados en el año y sin matrícula vigente", "N° Estudiantes 2022 no matriculados en 2023", "Detalle N° Estudiantes 2021 no matriculados ni en 2022 ni 2023")
 
 
 colnames(xls_desvinc_ee_cur) <- c("Región", "cod_deprov_rbd2022", "Nombre Deprov", "Comuna", "Nombre Sostenedor", "RUT Sostenedor", "email Sostenedor",
                                   "Teléfono Sostenedor", "Dependencia", "RBD", "Nombre RBD", "Último grado registrado", "N° Total Estudiantes sin matricula vigente",
                                  ...
                                  # "Teléfono Sostenedor", "Dependencia", "RBD", "Nombre RBD", "Último grado registrado", "Matrícula 2023 + estudiantes sin matrícula", "N° Estudiantes 2022-2023 sin matricula vigente", "% Estudiantes 2022-2023 sin matricula vigente",
                                  # "N° Estudiantes 2023 retirados en marzo y sin matrícula vigente", "N° Estudiantes 2022 no matriculados en 2023", "Detalle N° Estudiantes 2021 no matriculados ni en 2022 ni 2023")
 ...
 cods_deprov <- unique(bd_asis$cod_deprov_rbd)
 noms_deprov <- unique(bd_asis$nom_deprov_rbd)
 noms_deprov
 
 table(bd_asis$cod_deprov_rbd, bd_asis$nom_deprov_rbd)
 
 i = 0
 nn = length(cods_deprov)
 nn
 cods_deprov[1:5]
 ...
 table(bd_asis$cod_depe2, useNA = "a")
 table(bd$cod_reg_rbd_2022r, useNA = "a")
 # bd$nom_com_rbd_2022r
 
 colnames(bd_asis)
 aa<- bd_asis %>% group_by(nom_deprov_rbd, cod_deprov_rbd) %>% summarise(n())
 
#  cod_deprov <- cods_deprov[11]
 for(cod_deprov in cods_deprov){     #c(42,53,101,136,134)
 try({
  {
     tic()
     #**************************************************************************************************************************/
     ...
     #Se sacan las comunas de la deprov a partir de asistencia porque la de desvinculados interanual tiene errores en ciertas deprov/comunas
     comunas_deprov <- unique(bd_asis[bd_asis$cod_deprov_rbd == cod_deprov, nom_com_rbd])
     
     desvinc <-   bd[bd$nom_com_rbd_2022r %in% comunas_deprov,] #bd[bd$cod_deprov_rbd2022 == cod_deprov,]
     nrow(desvinc)
     data_plot2 <- data.frame(nom_grado = unique(desvinc$nom_grado))
     
     
     data_plot4 <- data_plot4_n %>% filter(nom_com_rbd_2022r %in% comunas_deprov)    #(nom_com_rbd_2022r %in% unique(desvinc$nom_com_rbd_2022r)) #data.frame(nom_rbd2022r = unique(desvinc$nom_rbd2022r))
     
     
     #n_mat2022 <- xls_desvinc_ee[xls_desvinc_ee$Comuna %in% comunas_deprov,]
     ...
     desvinc2 <- bd2[bd2$cod_deprov_rbd_ret == cod_deprov,]
     desvinc2 <- desvinc2 %>% filter(categoria_desert == "Retirado 2023")
     desvinc3 <- doble_desvinc[doble_desvinc$cod_deprov_rbd == cod_deprov,]
     # 
     # n_desvinc <- nrow(desvinc)
     # 
     # 
     # n_desvinc2 <- xls_desvinc_ee[xls_desvinc_ee$cod_deprov_rbd2022 == cod_deprov,]
     ...
     ###PRUEBA
     n_desvinc <- xls_desvinc_ee[xls_desvinc_ee$cod_deprov_rbd2022 == cod_deprov,]
     ...
     n_desvinc3 <- xls_desvinc_ee[xls_desvinc_ee$cod_deprov_rbd2022 == cod_deprov,]
     ...
     asis_crit <- bd_asis[bd_asis$cod_deprov_rbd == cod_deprov,]
     
     data_plot2_2 <- data.frame(nom_grado = unique(desvinc2$nom_grado_ret))
     data_plot4_2 <- data_plot4_n_2 %>% filter(cod_com_rbd %in% unique(desvinc2$cod_com_rbd_ret)) #data.frame(nom_rbd2022r = unique(desvinc$nom_rbd2022r))
     ...
     nom_deprov <- asis_crit$nom_deprov_rbd[1]
     #n_mat2023 = nrow(asis_crit)
     # n_mat2023 = xsl_asis_ee[xsl_asis_ee$cod_deprov_rbd == cod_deprov,] #nrow(asis_crit)
     # n_mat2023 = sum(n_mat2023$`N° Estudiantes totales`, na.rm = T)
     # 
     # n_mat2023_ret <- xls_desvinc_ee[xls_desvinc_ee$cod_deprov_rbd2022 == cod_deprov,]
     ...
     n_mat2023 = xls_desvinc_ee[xls_desvinc_ee$cod_deprov_rbd2022 == cod_deprov,] #nrow(asis_crit)
     ...
     #n_mat2023 = nrow(asis_crit)
     n_mat_asis2023 = xsl_asis_ee[xsl_asis_ee$cod_deprov_rbd == cod_deprov,] #nrow(asis_crit)
     ...
     #BD resumen EE
     resumen_asis_sost <- resumen_asis_ee %>% filter(cod_deprov_rbd == cod_deprov)
     ...
     print("hola")
     resumen_rbd_ee <- resumen_rbd %>% filter(cod_deprov_rbd == cod_deprov)
     # print("chasaaao")
     resumen_rbd_ee <- resumen_rbd_ee %>% select(-c("cod_deprov_rbd"))
     # print("chao")
     
     print(resumen_asis_sost)
     print("-----")  
     resumen_desvinc_sost <- resumen_desvinc_ee %>% filter(cod_deprov_rbd2022 == cod_deprov & (desv_total23 > 0))
     print(resumen_desvinc_sost)
     print("***")
     
     #Datos gráfico de barra de rangos de asistencia   #tagdiferencia (2) --> listo. Se agrega la categoría sin información.
     ...
     if(nrow(asis_crit) > 0){
       ...
       data_plot5_t <- data.frame(tipo_asis_2= c("Inasist. crítica (0%-49% asist.)", "Inasist. grave (50%-84% asist.)", "Inasist. reiterada (85%-89% asist.)", "Asist. esperada (90%-100% asist.)", "Sin información"), prop = c(0,0,0,0,0))     #tagdiferencia (2) --> listo. Se agrega la categoría sin información.
     }
     data_plot5_t 
     ...
     p <- plot_ly(data_plot1, labels = ~sit_fin_r_2022r, values = ~round(prop,3), type = 'pie', hole = 0, textposition = 'outside', 
       ...
       layout(legend = list(font = list(family = "verdana", size = 18, color = azul), orientation = "h", x=0.1 , y=1.4), font=list(family = "verdana", size = 17, color = azul), xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
              yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE), 
              paper_bgcolor='transparent', autosize = TRUE
       )
     
     # url_sin_fin_g = paste0("/Img temp/",cod_deprov,"_sit_fin_g.png")
     # orca(p, url_sin_fin_g)
     
     url_sin_fin_g = paste0(getwd(),"/Img temp/",cod_deprov,"_sit_fin_g.png")
     ...
     # url_cur_desv_g = paste0("/Img temp/", cod_deprov,"_cur_desv_g.png")
     # orca(k, url_cur_desv_g)
     
     url_cur_desv_g = paste0(getwd(),"/Img temp/", cod_deprov,"_cur_desv_g.png")
     ...
     # url_cur_desv_g_2 = paste0("/Img temp/", cod_deprov,"_cur_desv_g_2.png")
     # orca(k2, url_cur_desv_g_2)
     
     url_cur_desv_g_2 = paste0(getwd(),"/Img temp/", cod_deprov,"_cur_desv_g_2.png")
     ...
     # url_asis_crit_g = paste0("/Img temp/", cod_deprov,"_asis_crit_g.png")
     # orca(j, url_asis_crit_g)
     
     url_asis_crit_g = paste0(getwd(),"/Img temp/", cod_deprov,"_asis_crit_g.png")
     save_image(j, url_asis_crit_g)
     
     ########## GRAFICO BARRA DESVINCULADOS POR Dependencia.
     
     data_plot4_t <- desvinc %>% group_by(nom_com_rbd_2022r, cod_depe2_2022r) %>% summarise(n = n())
     
     print(data_plot4)
     print(data_plot4_t)
     data_plot4  <- left_join(data_plot4, data_plot4_t, by=c("nom_com_rbd_2022r", "cod_depe2_2022r"))
     ...
     m <- plot_ly() %>% add_trace(x = ~data_plot4$n, y = ~data_plot4$nom_com_rbd_2022r, color = ~data_plot4$cod_depe2_2022r, colors = c("#004677", "#6bccd6", "#e94860", "#613b75", "#f3d03e"), type = "bar", orientation = "h", text = paste0("<b style='color:#007096'>",data_plot4$n,"</b>", "<i style='color:#007096';> (", data_plot4$porc_desvinc,"%)</i>"), textposition = "outside", textfont = list(family = "verdana", color = '#FFFFFF'))
     
     
     if(nrow(data_plot4) < 4){
       m <- m %>% layout(font = list(family = "verdana", color = '#007096', size = 17), title = "", bargap = 0.8,
                         xaxis = list(title="<b>N° Estudiantes 2022 no matriculados en 2023</b> <i>(% respecto a matrícula 2022)</i>", font = list(family = "verdana", size = 15), showlegend = FALSE, showgrid = FALSE, showticklabels = FALSE, side = "top", range = list(0, max(data_plot4$n)*1.18)),
     ...
     # url_sost_desv_g = paste0("/Img temp/", cod_deprov,"_sost_desv_g.png")
     # orca(m, url_sost_desv_g)
     
     url_sost_desv_g = paste0(getwd(),"/Img temp/", cod_deprov,"_sost_desv_g.png")
     save_image(m, url_sost_desv_g)
     
     ########## GRAFICO BARRA RETIRADOS POR DEPENDENCIA.
     
     data_plot4_t_2 <- desvinc2 %>% group_by(nom_com_rbd_ret, cod_depe2_ret) %>% summarise(n = n())
     data_plot4_2
     data_plot4_2  <- left_join(data_plot4_2, data_plot4_t_2, by=c("nom_com_rbd"="nom_com_rbd_ret", "cod_depe2"="cod_depe2_ret"))
     data_plot4_2
     ...
     m2 <- plot_ly() %>% add_trace(x = ~data_plot4_2$n, y = ~data_plot4_2$nom_com_rbd, color = ~data_plot4_2$cod_depe2, colors = c("#004677", "#6bccd6", "#e94860", "#613b75", "#f3d03e"), type = "bar", orientation = "h", text = paste0("<b style='color:#007096'>",data_plot4_2$n,"</b>", "<i style='color:#007096';> (", data_plot4_2$porc_desvinc,"%)</i>"), textposition = "outside", textfont = list(family = "verdana", color = '#FFFFFF'))
     
     
     if(nrow(data_plot4_2) < 4){
       m2 <- m2 %>% layout(font = list(family = "verdana", color = '#007096', size = 17), title = "", bargap = 0.8,
                           xaxis = list(title="<b>N° Estudiantes retirados durante 2023 y sin matrícula actual</b> <i>(% respecto a matrícula 2022)</i>", font = list(family = "verdana", size = 15), showlegend = FALSE, showgrid = FALSE, showticklabels = FALSE, side = "top", range = list(0, max(data_plot4_2$n)*1.18)),
                           yaxis = list(title="<b>Comuna</b>", font = list(family = "verdana", size = 17), showlegend = FALSE, categoryorder = "total ascending"),
                           plot_bgcolor='transparent', paper_bgcolor='transparent', autosize = F, width = 1200, height = 800, margin = list(l=-0.1, t = -0.1, r=-0.1)
       )
     } else{
     ...
     # url_sost_desv_g_2 = paste0("/Img temp/", cod_deprov,"_sost_desv_g_2.png")
     # orca(m2, url_sost_desv_g_2)
     
     url_sost_desv_g_2 = paste0(getwd(),"/Img temp/", cod_deprov,"_sost_desv_g_2.png")
     ...
     # url_sost_asist_g = paste0("/Img temp/", cod_deprov,"_sost_asist_g.png")
     # orca(o, url_sost_asist_g)
     
     url_sost_asist_g = paste0(getwd(),"/Img temp/", cod_deprov,"_sost_asist_g.png")
     save_image(o, url_sost_asist_g)
     
     ########## GRAFICO BARRA ASIS CRIT POR EE DEL SOSTENEDOR.
     
     data_plot6_t <- asis_crit %>% group_by(nom_com_rbd, cod_depe2) %>% summarise(n = n()) %>% ungroup()
     data_plot6  <- left_join(data_plot6, data_plot6_t, by=c("nom_com_rbd", "cod_depe2"))
     ...
     q <- plot_ly() %>% add_trace(x = ~data_plot6$porc_asiscrit, y = ~data_plot6$nom_com_rbd,  color = ~data_plot6$cod_depe2, colors = c("#004677", "#6bccd6", "#e94860", "#613b75", "#f3d03e"), type = "bar", orientation = "h", text = paste0("<b style='color:#007096'>", data_plot6$porc_asiscrit, "%","</b>", "<i style='color:#007096';> (", data_plot6$n," de ", data_plot6$n_total,")</i>"), textposition = "outside", textfont = list(family = "verdana", color = '#FFFFFF'))
     ...
     # url_sost_asisgrave_g = paste0("/Img temp/", cod_deprov,"_sost_asisgrave_g.png")
     # orca(q, url_sost_asisgrave_g)
     
     url_sost_asisgrave_g = paste0(getwd(),"/Img temp/", cod_deprov,"_sost_asisgrave_g.png")
     ...
     #*
     # Sys.setlocale("LC_ALL","Spanish")
     rmarkdown::render("Codigos/DEPROV/RMKD Informe Deprov (EPJA).Rmd",
                       params = list("cod_deprov" = cod_deprov, "nom_deprov" = nom_deprov, "desvinc" = desvinc, "desvinc2" = desvinc2, "asis_crit" = asis_crit, "url_sin_fin_g" = url_sin_fin_g, "url_cur_desv_g" = url_cur_desv_g, "url_asis_crit_g" = url_asis_crit_g, "url_sost_desv_g" = url_sost_desv_g, "url_sost_desv_g_2" = url_sost_desv_g_2, "url_cur_desv_g_2" = url_cur_desv_g_2, "url_sost_asist_g" = url_sost_asist_g,
                                     "url_sost_asisgrave_g" = url_sost_asisgrave_g, "n_desvinc3" = n_desvinc3,"n_desvinc" = n_desvinc, "n_desvinc2" = n_desvinc2, "n_mat2023" = n_mat2023, "n_mat_asis2023" = n_mat_asis2023, "n_inasis2023" = n_inasis2023, "resumen_asis_sost" = resumen_asis_sost, "resumen_desvinc_sost" = resumen_desvinc_sost, "resumen_rbd" = resumen_rbd_ee),
                       output_file = paste0("Outputs/Deprov/Deprov (EPJA)/", nom_deprov, " (EPJA).pdf"))
     print(paste0("Listo ", i ," de ", nn, " Deprov. (Codigo: ", cod_deprov, ")"))
     ...
     xsl_asis_ee_deprov <- xsl_asis_ee %>% filter(cod_deprov_rbd == cod_deprov) %>% select(-c(cod_deprov_rbd))
     xsl_asis_ee_cur_deprov <- xsl_asis_ee_cur %>% filter(cod_deprov_rbd == cod_deprov) %>% select(-c(cod_deprov_rbd))
     xls_desvinc_ee_deprov <- xls_desvinc_ee %>% filter(Comuna %in% comunas_deprov) %>% select(-c(cod_deprov_rbd2022))
     xls_desvinc_ee_cur_deprov <- xls_desvinc_ee_cur %>% filter(Comuna %in% comunas_deprov) %>% select(-c(cod_deprov_rbd2022))
     ...
     #addStyle(wb, "Detalle no_matric x RBD y grado", style=pct, cols=c(14), rows=2:(nrow(xls_desvinc_ee_cur_deprov)+1), gridExpand=TRUE)
     addStyle(wb, "Resumen asist x RBD", style=pct, cols=c(11:17, 20, 26:31), rows=2:(nrow(xsl_asis_ee_deprov)+1), gridExpand=TRUE)
     addStyle(wb, "Detalle asist x RBD y grado", style=pct, cols=c(12:18, 21, 26:30), rows=2:(nrow(xsl_asis_ee_cur_deprov)+1), gridExpand=TRUE)
     
     
     saveWorkbook(wb, paste0("Outputs/Deprov/Deprov (EPJA)/", nom_deprov, " (EPJA).xlsx"), overwrite = T)
