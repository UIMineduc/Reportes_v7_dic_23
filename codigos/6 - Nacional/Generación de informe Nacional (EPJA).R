...
p_load(tidyverse, rio, dplyr, ggplot2, stringr, tidyr, kableExtra, 
       ...
       tictoc, htmlwidgets, webshot, tinytex, openxlsx, pander)
...
# -------------------------------------------------------------------------
 ...
 bd_rbd_rutsost <- bd_asis %>% group_by(rut_sost_rbd2022, nombre_sost_rbd2022, rbd) %>% slice(1) %>% ungroup() %>% select(rut_sost_rbd2022, nombre_sost_rbd2022, email_sost_rbd2022, tel_sost_rbd, rbd)  #tagnuevo se agregan campos de retirados
...
table(bd_asis$cod_reg_rbd2022r2, useNA = "a")
table(bd$cod_reg_rbd2022r, useNA = "a")

data_plot4_n <- bd %>% group_by(cod_reg_rbd2022r2) %>% summarise(n_total = n()) %>% ungroup()  #**
data_plot4_n_2 <- bd_asis %>% group_by(cod_reg_rbd2022r2) %>% summarise(n_total = n()) %>% ungroup()   #**
bd_asis <- bd_asis %>% filter(cod_depe2 != 3)
data_plot6_n <- bd_asis %>% group_by(cod_reg_rbd2022r2) %>% summarise(n_total = n()) %>% ungroup()   #**
...
resumen_asis_ee <- bd_asis %>% filter(cod_depe2 != 3) %>% group_by(cod_reg_rbd2022r2, nom_com_rbd, cod_com_rbd) %>% summarise(asis_marzo= mean(porcentage_asistencia_marzo, na.rm = TRUE), 
...
resumen_asis_ee2 <- bd_asis %>% filter(cod_depe2 != 3) %>% group_by(cod_reg_rbd2022r2, nom_com_rbd, cod_com_rbd, asistencia_categorias_acumulada) %>% summarise(n_cat = n())
...
resumen_asis_ee = left_join(resumen_asis_ee, resumen_asis_ee2, by = c("cod_reg_rbd2022r2", "nom_com_rbd", "cod_com_rbd"))
...
resumen_asis_ee <- resumen_asis_ee %>% arrange(cod_reg_rbd2022r2, nom_com_rbd)
...
n_ret_rbd <- bd2 %>%  group_by(cod_reg_rbd_ret) %>% summarise(n_ret = sum(retirados, na.rm = T)) %>% ungroup() %>% select(cod_reg_rbd_ret, n_ret)
data_plot4_n_2 <- left_join(data_plot4_n_2, n_ret_rbd, by=c("cod_reg_rbd2022r2" = "cod_reg_rbd_ret"))
...
mat_2023_com_cur <- bd_asis_todos %>% group_by(cod_reg_rbd2022r2, nom_com_rbd, cod_com_rbd) %>% summarise(n_total23 = n()) %>% ungroup()
mat_2023_com_cur <- mat_2023_com_cur %>% rename(cod_reg_rbd2022r2 = cod_reg_rbd2022r2, nom_com_rbd_2022r = nom_com_rbd, cod_com_rbd_2022r = cod_com_rbd)

table(bd_asis$cod_reg_rbd2022r2)
...
resumen_desvinc_ee <- bd  %>%  group_by(cod_com_rbd_2022r) %>% summarise(n_total = n(), n_desv = sum(desvinculados, na.rm = T))#summarise(n_total = n(), n = sum(desert_glob_total_aju_marzo_2023))
resumen_desvinc_ee2 <- bd2 %>% group_by(cod_com_rbd_ret) %>% summarise(n_ret = sum(retirados, na.rm = T)) %>% ungroup() #%>% select(cod_reg_rbd_ret, n_ret) #summarise(n_total = n(), n = sum(desert_glob_total_aju_marzo_2023))
resumen_dobledesvinc_ee <- doble_desvinc %>% group_by(cod_com_rbd) %>% summarise(n_dobledesv = n()) #%>% ungroup() %>% select(rbd, n_dobledesv)#summarise(n_total = n(), n = sum(desert_glob_total_aju_marzo_2023))

resumen_desvinc_ee <- full_join(mat_2023_com_cur, resumen_desvinc_ee, by = c("cod_com_rbd_2022r" = "cod_com_rbd_2022r"))
resumen_desvinc_ee <- full_join(resumen_desvinc_ee, resumen_desvinc_ee2, by = c("cod_com_rbd_2022r" = "cod_com_rbd_ret"))
resumen_desvinc_ee <- left_join(resumen_desvinc_ee, resumen_dobledesvinc_ee, by = c("cod_com_rbd_2022r" = "cod_com_rbd"))
...
resumen_desvinc_ee <- resumen_desvinc_ee %>% arrange(cod_reg_rbd2022r2, nom_com_rbd_2022r)
...
tbl_asis_ee <- xsl_asis_ee %>% select(cod_reg_rbd2022r2, nom_com_rbd, cod_depe2, rbd, nom_rbd, nombre_sost_rbd2022, asis_marzo, 
  ...
  # xsl_asis_ee <- xsl_asis_ee %>% mutate_if(is.character, ~enc2utf8(.))
  # xsl_asis_ee_cur <- xsl_asis_ee_cur %>% mutate_if(is.character, ~enc2utf8(.))
  # xls_desvinc_ee <- xls_desvinc_ee %>% mutate_if(is.character, ~enc2utf8(.))
  # xls_desvinc_ee_cur <- xls_desvinc_ee_cur %>% mutate_if(is.character, ~enc2utf8(.))
    ...
    
    {
      ...
      cod_reg <- cods_reg
       ...
       #  try({
            ...
            desvinc <- bd[bd$cod_reg_rbd2022r2 %in% cod_reg,]
            nrow(desvinc)
            data_plot2 <- data.frame(nom_grado = unique(desvinc$nom_grado))
    
            data_plot4 <- data_plot4_n %>% filter(cod_reg_rbd2022r2 %in% unique(desvinc$cod_reg_rbd2022r2)) #data.frame(nom_rbd2022r = unique(desvinc$nom_rbd2022r))
            ...
            desvinc2 <- bd2[bd2$cod_reg_rbd_ret %in% cod_reg,]
            desvinc2 <- desvinc2 %>% filter(categoria_desert == "Retirado 2023")
            desvinc3 <- doble_desvinc[doble_desvinc$cod_reg_rbd %in% cod_reg,]
            ...
            ###PRUEBA
            n_desvinc <- xls_desvinc_ee[xls_desvinc_ee$cod_reg_rbd2022r2 %in% cod_reg,]
            n_desvinc = sum(n_desvinc$`N° Estudiantes 2022 no matriculados en 2023`, na.rm = T) # nrow(desvinc2)
  
            n_desvinc2 <- xls_desvinc_ee[xls_desvinc_ee$cod_reg_rbd2022r2 %in% cod_reg,]
            n_desvinc2 = sum(n_desvinc2$`N° Estudiantes 2023 retirados en el año y sin matrícula vigente`, na.rm = T) # nrow(desvinc2)
  
            n_desvinc3 <- xls_desvinc_ee[xls_desvinc_ee$cod_reg_rbd2022r2 %in% cod_reg,]
            ...
            asis_crit <- bd_asis[bd_asis$cod_reg_rbd2022r2 %in% cod_reg,]
            table(bd_asis$cod_reg_rbd, useNA = "a")
            # n_mat_asis2023 = nrow(asis_crit)
            
            data_plot2_2 <- data.frame(nom_grado_ret = unique(desvinc2$nom_grado_ret))
            data_plot4_2 <- data_plot4_n_2 %>% filter(cod_reg_rbd2022r2 %in% unique(desvinc2$cod_reg_rbd_ret)) #data.frame(nom_rbd2022r = unique(desvinc$nom_rbd2022r))
            
            data_plot3 <- data.frame(cod_curso = unique(asis_crit$cod_curso))
            data_plot3_n <- asis_crit %>% group_by(cod_curso) %>% summarise(n_total = n())
            data_plot6 <- data_plot6_n %>% filter(cod_reg_rbd2022r2 %in% unique(asis_crit$cod_reg_rbd2022r2))
            ...
            n_mat2023 = xls_desvinc_ee[xls_desvinc_ee$cod_reg_rbd2022r2 %in% cod_reg,] #nrow(asis_crit)
            ...
            n_mat_asis2023 = xsl_asis_ee[xsl_asis_ee$cod_reg_rbd2022r2 %in% cod_reg,] #nrow(asis_crit)
            ...
            #BD resumen EE
            resumen_asis_sost <- resumen_asis_ee %>% filter(cod_reg_rbd2022r2 %in% cod_reg)
            
            resumen_asis_sost <- resumen_asis_sost %>% select(cod_reg_rbd2022r2, nom_com_rbd, cod_com_rbd, asis_promedio, asis_marzo, 
                                                              asis_abril, asis_mayo,
                                                              asis_junio, asis_julio, asis_agosto,
                                                              #asis_septiembre, asis_octubre, asis_noviembre, 
            ...
            resumen_asis_sost <- resumen_asis_sost %>% arrange(cod_reg_rbd2022r2, nom_com_rbd)
            
            
            
            print("hola")
            resumen_rbd_ee <- resumen_rbd %>% filter(cod_reg_rbd2022r2 %in% cod_reg)
            ...
            print("-----")  
            resumen_desvinc_sost <- resumen_desvinc_ee %>% filter(cod_reg_rbd2022r2 %in% cod_reg & (n_desv > 0 | n_ret > 0))
            ...
            n_inasis2023 = xsl_asis_ee[xsl_asis_ee$cod_reg_rbd2022r2 %in% cod_reg,] #nrow(asis_crit)
            ...
            asis_crit <- asis_crit %>% select("cod_reg_rbd2022r2", "run_alu", "nom_alu", "app_alu", "apm_alu", "cod_depe2", "cod_curso", "rbd", "nom_rbd", "nom_com_rbd", "nom_reg_rbd_a", "porcentage_asistencia_acumulada", "inasistencia_grave_acumulada", "asistencia_categorias_acumulada", "tipo_asis_2")
            asis_crit <- asis_crit %>% arrange(cod_reg_rbd2022r2, cod_curso, porcentage_asistencia_acumulada)
            nrow(asis_crit)
            ...
            p <- plot_ly(data_plot1, labels = ~sit_fin_r_2022r, values = ~round(prop,3), type = 'pie', hole = 0, textposition = 'outside', 
              ...
              layout(legend = list(font = list(family = "verdana", size = 18, color = azul), orientation = "h", x=0.1 , y=1.2), font=list(family = "verdana", size = 17, color = azul), xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
                     yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE), 
                     paper_bgcolor='transparent', autosize = TRUE
              )
            
            # url_sin_fin_g = paste0("/Img temp/","nacional","_sit_fin_g.png")
            # orca(p, url_sin_fin_g)
            
            url_sin_fin_g = paste0(getwd(),"/Img temp/","nacional","_sit_fin_g.png")
          ...
          # url_cur_desv_g = paste0("/Img temp/", "nacional","_cur_desv_g.png")
          # orca(k, url_cur_desv_g)
          
          url_cur_desv_g = paste0(getwd(),"/Img temp/", "nacional","_cur_desv_g.png")
          ...
          # url_cur_desv_g_2 = paste0("/Img temp/", "nacional","_cur_desv_g_2.png")
          # orca(k2, url_cur_desv_g_2)
          
          url_cur_desv_g_2 = paste0(getwd(),"/Img temp/", "nacional","_cur_desv_g_2.png")
          ...
          # url_asis_crit_g = paste0("/Img temp/", "nacional","_asis_crit_g.png")
          # orca(j, url_asis_crit_g)
          
          url_asis_crit_g = paste0(getwd(),"/Img temp/", "nacional","_asis_crit_g.png")
          save_image(j, url_asis_crit_g)
          
          ########## GRAFICO BARRA DESVINCULADOS POR Dependencia.
        
          data_plot4_t <- desvinc %>% group_by(cod_reg_rbd2022r2) %>% summarise(n = n())
        
          print(data_plot4)
          print(data_plot4_t)
          data_plot4  <- left_join(data_plot4, data_plot4_t, by=c("cod_reg_rbd2022r2"))
          ...
          m <- plot_ly() %>% add_trace(x = ~data_plot4$n, y = ~data_plot4$cod_reg_rbd2022r2, marker = list(color = "#6bccd6"), type = "bar", orientation = "h", text = paste0("<b style='color:#007096'>",data_plot4$n,"</b>", "<i style='color:#007096';> (", data_plot4$porc_desvinc,"%)</i>"), textposition = "outside", textfont = list(family = "verdana", color = '#FFFFFF'))
          ...
          # url_sost_desv_g = paste0("/Img temp/", "nacional","_sost_desv_g.png")
          # orca(m, url_sost_desv_g)
          
          url_sost_desv_g = paste0(getwd(),"/Img temp/", "nacional","_sost_desv_g.png")
          ...
          data_plot4_t_2 <- desvinc2 %>% group_by(cod_reg_rbd_ret) %>% summarise(n = n())
          data_plot4_2
          data_plot4_2  <- left_join(data_plot4_2, data_plot4_t_2, by=c("cod_reg_rbd2022r2"="cod_reg_rbd_ret"))
          data_plot4_2
          ...
          m2 <- plot_ly() %>% add_trace(x = ~data_plot4_2$n, y = ~data_plot4_2$cod_reg_rbd2022r2, marker = list(color = "#6bccd6"), type = "bar", orientation = "h", text = paste0("<b style='color:#007096'>",data_plot4_2$n,"</b>", "<i style='color:#007096';> (", data_plot4_2$porc_desvinc,"%)</i>"), textposition = "outside", textfont = list(family = "verdana", color = '#FFFFFF'))
          ...
          if(nrow(data_plot4_2) < 4){
            m2 <- m2 %>% layout(font = list(family = "verdana", color = '#007096', size = 17), title = "", bargap = 0.8,
                              ...
                              yaxis = list(title="<b>Región</b>", font = list(family = "verdana", size = 17), showlegend = FALSE, categoryorder = "total ascending"),
                              plot_bgcolor='transparent', paper_bgcolor='transparent', autosize = F, width = 1200, height = 800, margin = list(l=-0.1, t = -0.1, r=-0.1)
            )
          ...
          # url_sost_desv_g_2 = paste0("/Img temp/", "nacional","_sost_desv_g_2.png")
          # orca(m2, url_sost_desv_g_2)
          
          url_sost_desv_g_2 = paste0(getwd(),"/Img temp/", "nacional","_sost_desv_g_2.png")
          ...
          # url_sost_asist_g = paste0("/Img temp/", "nacional","_sost_asist_g.png")
          # orca(o, url_sost_asist_g)
          
          url_sost_asist_g = paste0(getwd(),"/Img temp/", "nacional","_sost_asist_g.png")
          ...
          data_plot6_t <- asis_crit %>% group_by(cod_reg_rbd2022r2) %>% summarise(n = n()) %>% ungroup()
          data_plot6  <- left_join(data_plot6, data_plot6_t, by=c("cod_reg_rbd2022r2"))
          ...
          # url_sost_asisgrave_g = paste0("/Img temp/", "nacional","_sost_asisgrave_g.png")
          # orca(q, url_sost_asisgrave_g)
          
          url_sost_asisgrave_g = paste0(getwd(),"/Img temp/", "nacional","_sost_asisgrave_g.png")
          ...
          rmarkdown::render("Codigos/Nacional/RMKD Informe Nacional (EPJA).Rmd",
                            params = list("cod_deprov" = "Nacional", "nom_deprov" = "Nacional", "desvinc" = desvinc, "desvinc2" = desvinc2, "desvinc3" = desvinc3, "asis_crit" = asis_crit, "url_sin_fin_g" = url_sin_fin_g, "url_cur_desv_g" = url_cur_desv_g, "url_asis_crit_g" = url_asis_crit_g, "url_sost_desv_g" = url_sost_desv_g, "url_sost_desv_g_2" = url_sost_desv_g_2, "url_cur_desv_g_2" = url_cur_desv_g_2, "url_sost_asist_g" = url_sost_asist_g,
                                          "url_sost_asisgrave_g" = url_sost_asisgrave_g, "n_desvinc" = n_desvinc, "n_desvinc2" = n_desvinc2, "n_desvinc3" = n_desvinc3, "n_mat2023" = n_mat2023, "n_mat_asis2023" = n_mat_asis2023, "n_inasis2023" = n_inasis2023, "resumen_asis_sost" = resumen_asis_sost, "resumen_desvinc_sost" = resumen_desvinc_sost, "resumen_rbd" = resumen_rbd_ee, "pass" = pass),
                            output_file = paste0("Outputs/Nacional (EPJA)/", "Nacional (EPJA)", ".pdf"))
      ...
      xsl_asis_ee_deprov <- xsl_asis_ee %>% ungroup() %>% filter(cod_reg_rbd2022r2 %in% cod_reg) %>% select(-c("cod_deprov_rbd")) %>% rename("Región" = "cod_reg_rbd2022r2")
      xsl_asis_ee_cur_deprov <- xsl_asis_ee_cur %>% ungroup() %>% filter(cod_reg_rbd2022r2 %in% cod_reg) %>% select(-c("cod_deprov_rbd")) %>% rename("Región" = "cod_reg_rbd2022r2")
      xls_desvinc_ee_deprov <- xls_desvinc_ee %>% ungroup() %>% filter(cod_reg_rbd2022r2 %in% cod_reg) %>% select(-c("cod_deprov_rbd2022")) %>% rename("Región" = "cod_reg_rbd2022r2")
      xls_desvinc_ee_cur_deprov <- xls_desvinc_ee_cur %>% ungroup() %>% filter(cod_reg_rbd2022r2 %in% cod_reg) %>% select(-c("cod_deprov_rbd2022")) %>% rename("Región" = "cod_reg_rbd2022r2")
      ...
      addStyle(wb, "Resumen no_matric x RBD", style=pct, cols=c(13), rows=2:(nrow(xls_desvinc_ee_deprov)+1), gridExpand=TRUE)
      # addStyle(wb, "Detalle no_matric x RBD y grado", style=pct, cols=c(13), rows=2:(nrow(xls_desvinc_ee_cur_deprov)+1), gridExpand=TRUE)
      addStyle(wb, "Resumen asist x RBD", style=pct, cols=c(11:17, 20, 23:31), rows=2:(nrow(xsl_asis_ee_deprov)+1), gridExpand=TRUE)
      addStyle(wb, "Detalle asist x RBD y grado", style=pct, cols=c(12:18, 21, 27:32), rows=2:(nrow(xsl_asis_ee_cur_deprov)+1), gridExpand=TRUE)
      
      wb
      saveWorkbook(wb, paste0("Outputs/Nacional (EPJA)/", "Nacional", " (EPJA).xlsx"), overwrite = T)
